...
| `10:10+n` | n | `name` | **latin-1**, max 16 chars, NUL-terminated. Not ASCII: 0x7F is authored, as the separator before a stereo pair's `L`/`R` marker (`VOI:Attack Voi\x7fL`) — 4 073 occurrences across 4 627 local banks, all inside the 16-character field. (0x7F is ASCII, so it survived the old ASCII encode too; latin-1 is for exact inverse symmetry with the reader, and matters only for bytes ≥ 0x80.) See RESOLUTION_NOTES §NAMEBYTE. A name that fills the field exactly has no terminator, so a reader must cap at 16 rather than run to `ofs` — `ofs` is padded and rounded, and reading to it returns trailing block bytes as if they were name characters (real banks: `b'General MIDI kit\x9d\xdb'`). See RESOLUTION_NOTES §KRZNAME16; those two bytes were briefly counted as evidence of a non-ASCII character set. |
