...
| **EB16 / effects: the bus is NEVER selected in six library discs** | *(filed 2026-09-10; first figures were wrong and are corrected here)* **Status: OPEN as a feature, but the prevalence is ZERO, not the 1.8% first reported.** 2292 programs / 8573 keygroups, exact-S3000 layout only: <br><br>`PFXCHAN` — 2180 hold **0**, 71 hold **255** (the machine's OFF convention), and the remaining **41 are OUT OF RANGE** (values 10, 29, 30 against a legal 0–4). **Not one program selects a legal effects bus.** <br>`KFXCHAN` — 8439 in range and **every one of them is 0**; 134 are out of range. **Not one keygroup overrides the bus.** <br>`KFXSLEV` — 114 keygroups carry a legal non-zero send level, with no bus ever selected. `PFXSLEV` likewise, on 190 programs, which matches the spec's own `desc="Not used"`. <br><br>**THE FIRST PASS COUNTED NON-ZERO AND EXCLUDED 255, AND STOPPED THERE.** That gave "41 programs select a bus, 1.8%" and "144 keygroups override it, 1.7%" — every one of which was an out-of-range byte. **Checking that a value is non-zero and not the off-sentinel is not checking that it is legal**, and the out-of-range rate is the signal: 41 of 2292 and 134 of 8573 is the signature of unwritten bytes, the same shape as `FLT2MODE`'s 0.12%. <br><br>**Consequence for s3ked's `KFXCHAN` off-by-one** (byte 161 is documented twice, once with a leading `0 = PRG` that shifts every later value): **it affects zero material in this corpus**, because no keygroup carries a non-zero value at all. It remains a real spec ambiguity worth settling — the EB16 makes it audible for the first time — but it cannot be ranked by prevalence here. <br><br>**We still read and write none of these fields**, and the model has no effects-send field. A parser that does not silently drop a field is worth having regardless of the percentage; bench time for the enum is not, on this evidence. |
