...
| **AKAI: we write PMCHAN 255 (OMNI) on every program; factory material never does** | **Found 2026-08-18 by asking whether the corpus represents our conventions.** Across 11410 factory S3000 programs, **3 use OMNI — 0.03%.** We use it on 100%. The writer's reasoning is recorded and plausible (*"a converted program has no channel of its own, so omni is the sane default"*) and it was never checked against a real disc. **The PRGNUM half of this row is withdrawn:** 98.3% of 2408 factory volumes also start at PRGNUM 0, so the boot-program stack is ordinary AKAI behaviour and not our deviation. **What OMNI risks:** every resident program answers every channel, which is exactly the mechanism behind the retracted §135 stacking. Distinct PRGNUMs may make it harmless — untested. **One test settles it:** load a multi-program converted volume (VF NEW has 21) and confirm only the selected program sounds. Not blocked on RE. §AKAIRELTEST |
