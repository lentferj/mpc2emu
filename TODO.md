...
| **The KRZ writer destroys a K2000's own velocity->filter cord and smears it into the cutoff** | *(2026-08-25, KRZ corpus sweep)* `krz_writer._patch_layer` folds `max(0, velocity_to_filter)` into the static cutoff and writes no cord back, so a K2000 program read and rewritten loses the cord it had. **Measured on the 32-bank KRZ corpus: `velocity_to_filter` changes on 46.7% of zones (to 0.0), and neutralising the fold drops `filter_cutoff` from 49.3% to 3.5%** -- so the fold, not the cutoff scale, is what remains after §CUTOFFHZ. The fold is **correct for its original purpose**: an MPC source means `eff = cutoff + velToFilter` and a K2000 VelTrk sweep from the 16 Hz floor would mute softly-played notes the MPC keeps audible (comment at the site). It is wrong when the TARGET can express what the SOURCE said, which the K2000 can -- the reader already decodes the cord at `krz_parser.py:871/907`. **Not fixed in the same commit deliberately:** choosing between the fold and a real cord needs the source's semantics, and the MPC case that motivated the fold is the one that would regress. **Also seen in the same sweep:** a K2000 cutoff above 20 kHz (the byte reaches 25 kHz) is clamped to `E4B_CUTOFF_MAX_HZ` on the way through -- the model's ceiling imposed on a K2000->K2000 trip, the 3.5% residual above. See §KRZVELFOLD. |
