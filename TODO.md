...
| **EB16 / effects: the fields are unread, and the corpus says the ceiling is low** | *(filed 2026-09-10, board fitted the same evening)* **Status: OPEN, and the prevalence is recorded BEFORE anyone decides what to measure** — which is the correction this project learned the hard way on the IB-304F, where a bench decision was taken without a number that existed only on the converter side. Six library discs, **2293 S3000 programs / 8597 keygroups**: <br><br>`PFXCHAN` selects a bus on **41 programs — 1.8%** (and 41 of those also carry a send level). `KFXCHAN` overrides the bus on **144 keygroups — 1.7%**, 122 of them with a send level. **`PFXSLEV` is set on 190 programs (8.3%) that select no bus at all**, which fits its documentation as *"not used"* — a value the machine stores and ignores, and counting it as usage would triple the apparent stake. <br><br>**The raw non-zero counts were 2.7x higher until 255 was excluded**: 255 is OFF on this machine (the `OUTPUT` convention), so 71 of 112 "non-zero" `PFXCHAN` programs are explicitly saying *no effects bus*. Counting a disable as usage is exactly the error Jan's question caught on the IB-304F. <br><br>**We read and write none of it.** No parser touches `KFXCHAN`/`KFXSLEV`/`PFXCHAN`, the writer's field table carries `PFXSLEV` only as a documented no-op, and the model has no effects-send field on `VoiceLayer` or `Preset` at all (`chorus_amount` is unrelated). **So this is a feature to add, not a bug to fix** — and at ~1.8% of material it is worth less bench time than anything the filter board bought. |
