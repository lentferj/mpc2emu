...
| **`_or_default` exists precisely for this and the envelope path does not use it** | *(2026-08-24, code review)* `getattr(env,'decay',0.3) or 0.3` and `... or 0.5` treat a legitimate **0.0** as missing. A source stating an instant release (XPM `VolumeRelease` 0, sfz `ampeg_release=0`) gets the 0.5 s default -- RELSE1 ~56, a half-second tail on a patch that asked to stop dead. **[VERIFIED OPEN 2026-09-15]** Re-tested 2026-09-15: `akai_env_bytes` on an envelope with `release=0.0` still returns **RELSE1 byte 54** — a half-second tail on a patch that asked to stop dead. The same pattern was found and fixed elsewhere in this very file (the note at 3536: *"it went unnoticed because `or 0.5` rewrote the 0.0"*) and not here. |
