...
| **E4XT velocity→attack: the law is DERIVED, not measured — one bench run would confirm it** | *(2026-09-15, commissioned to eosed)* Nothing was invented, but nothing was measured either. The span law composes two independently measured results: **a cord spans its destination's range** (eosed §143, established on FMORPH because it is the only destination whose range is not 127 and so the only one that can tell that reading apart from "a cord is worth ~127 bytes") and **the attack byte is exponential in time** with `ENV_RATE_K`. Composed:<br><br>`span = exp(ENV_RATE_K × 1.27 × amount_percent)`, **independent of the base attack** — verified against the rate law to five digits at several bases, and a pleasant contrast with the FILTER cord, which is emphatically base-dependent. A ratio of times against an exponential byte law cancels the base; a span in cents against the filter's byte↔Hz curve does not.<br><br>Immune to one caveat worth recording: `ENV_RATE_A/K` were re-labelled this morning as **threshold** times rather than full-fall times. In a ratio of two times that fraction cancels exactly (checked at 1.0, 0.489 and 0.25 — the span does not move). What it is NOT immune to: the attack destination's range not being 127, `Vel<` not being linear in velocity, or the cord not adding to this byte the way it adds to FMORPH.<br><br>**Falsifiable and cheap:** amount 28 → span 7.89×, amount 50 → 40×. Two amounts, note-on at velocity 1 and 127, time the rise. |
