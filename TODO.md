...
| **`e4b_parser` reports every release ~3.9x too long** | *(2026-09-18)* `env_release = env_rate_to_span_seconds(_rel_span, pzt[8]) + _stage2_seconds(pzt[10])` asks how long segment 1's RATE would take to cross the WHOLE release span -- but the writer stops that segment at the knee. The over-count is `_rel_span / _r_mid_db` = 97.82/29 = **3.37x** on the first term, plus segment 2's time, giving the ~3.9x observed. Verified on a purpose-built ladder: 0.25/0.5/1/2/4 s written, read back as 0.971/1.919/3.974/7.854/15.686 -- a constant factor across a 16x range.<br><br>**WHICH HALF IS WRONG IS NOT YET PROVEN.** eosed's hardware read-back confirms the WRITER agrees with the MACHINE, which rules out writer-vs-machine disagreement and says nothing about writer-vs-SOURCE -- if the reader is right and the writer is 3.9x fast, the machine would agree with the writer exactly as observed. My inference that the reader was the wrong half was invalid and is withdrawn. **The discriminator is the MPC measurement above**, and on its evidence the writer is too SLOW (we reach 29 dB at 0.96R where the MPC reaches 60 dB at 0.95R), which points at the reader -- but that is an argument, not the arithmetic. Fifth writer/reader inverse-pair defect found today.<br><br>**Also open, from eosed:** they recalibrated the E4XT release byte -> dB/s law over bytes **44-86** and the version in our tree is up to **14 % out at byte 44 with the curvature sign inverted** (+0.000175 b^2 fitted over 58-86, -0.0000889 over 44-86). Both laws agree in 53-64 where tonight's work landed, so nothing measured today is affected -- but `e4b_writer` inverts that law to pick bytes, so it matters wherever a release is short. Their fit: `ln(dB/s) = -0.0000889 b^2 - 0.045224 b + 6.8949`, r2 0.99809. Not adopted; residuals not seen here. |
