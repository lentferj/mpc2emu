...
| **AKAI auxiliary file types are carried but not read** | *(unblocked 2026-08-17)* Every real library volume holds `.T`, `.X`, `.D` and `.M3` files; we name and round-trip them but read nothing. **The reference material this was blocked on now exists:** saving a volume on the S3000XL produced `TL1.T`, `EFFECTS FILE.X`, `DRUM INPUTS.D` and `MULTI FILE.M3` — machine-authored examples of all four, preserved at `/home/lentferj/temp/akai_resave_results/VOLUME_005/`. `.M3` is still the one worth doing (the AKAI multi). Low priority — E4B multisetups are not written either. Noted 2026-08-06, unblocked 2026-08-17 |
