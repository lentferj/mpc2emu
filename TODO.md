...
| ~~XPM `direction` (reverse playback) is dropped~~ | **fixed 2026-08-03** (branch `cwm_ketchup`) — reversal baked into the PCM after slicing, loop points mirrored. Byte-exact against a real reversed sample and mirror-exact in the audio domain. §XPMGAPS |
| ~~XPM loop crossfade is dropped~~ | **implemented 2026-08-03** (branch `cwm_ketchup`) — equal-power blend baked in, unit-verified byte-exact. **Caveat:** every layer in the corpus has crossfade 0/-1, so it has never run on real data and the MPC's own frame alignment is still unverified. §XPMGAPS |
| **XPM `loopFineTune` is ignored** | meaning unknown and `0` in all 69 808 corpus layers, so there is nothing to calibrate against. Warns if a non-zero one ever appears. **Blocked on:** a real file or a hardware test. §XPMGAPS |
| **XPM `ZonePlay = 2` (random) could map to EOS Crossfade Random** | currently warned and dropped. The EOS 4.0 manual p.320 documents Crossfade Random as *designed* for randomly switching between voices on the same key — a genuine equivalent. Needs realtime crossfade windows plus a cord per voice, so it is writer-side work wanting a hardware audition. `ZonePlay = 0` (cycle) has **no** EOS equivalent and stays dropped. §XPMGAPS |
