...
| **`filter_cutoff` is an E4B POSITION, so every AKAI corner below 57 Hz collapses to one byte** | *(2026-08-25, full corpus sweep)* The model stores cutoff as a position on the E-MU's 57 Hz..20 kHz scale. **Every AKAI FILFRQ below 28 maps to position 0 and comes back as 28** -- and FILFRQ < 28 is **29.4% of the corpus**. Measured on the sound libraries: `filter_freq` changed on **0.6% to 26.0% of zones per disc**, examples `21 -> 28`, `22 -> 28`, `23 -> 28`, `24 -> 28`. **This is the LAST of the three 'physical quantity in one machine's parameter scale' defects** -- the release span and the key-follow units were both fixed 2026-08-24/25 and both were costing about a third of the corpus in the same way. **Fix: store Hz** and convert at the point of use, exactly as `filter_keytrack` now does. Bigger than keytrack was: cutoff is consumed by every writer and set by every parser. **Also visible: `95 -> 94` at the top**, a separate one-byte search artefact. |
