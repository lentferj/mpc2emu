...
| **KRZ: a DSP block code is SLOT-RELATIVE, not global — F3 reading reverted** | An attempt to read tag `0x52` as a filter was **reverted 2026-08-17**: it invented filters. Byte 15 at `0x52` is a confirmed LOPASS in `' Computer Pad'` and a SHAPER in `' Sexy Beats'` and `" Echo's II"`, which the panel says have no filter at all — so a value test cannot separate them, which is exactly the reasoning behind the invented-bandpass bug. 58 of the 67 'recovered' programs carry that ambiguous code, so the published 1.6% exposure is an **upper bound**, not a count. **The K2000 manual gives the real structure** (`26 DSP Algs.pdf`, extracted to `/home/lentferj/temp/k2000_algorithms.json`, 31 algorithms with per-slot option lists): each algorithm fixes what each slot may hold, and algorithms 17/18 offer only `SHAPE MOD OSC`/`AMP MOD OSC`/`NONE` in the slot our `0x52` reads. So a block code means whatever its SLOT's option list says, and our global `_K2_FILTER_TO_XPM` is valid only for the slot it was validated on (F1, 581/581). **Blocked on:** correlating tag → manual slot number, which needs a PER-LAYER table since algorithms differ between layers of one program (`Deep +` is `[10, 24]`). A first attempt using layer 1's algorithm as ground truth scored 5/11 and 7/11 on two hypotheses and was malformed, not informative. §AKAIVELFILT |
