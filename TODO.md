...
| **Velocity → amp-envelope attack: AKAI done, E4B/XPM/KRZ still open** | *(2026-09-15, corpus-prioritised across all four formats before building)* Prevalence decided this: **AKAI 19.4 % of keygroups across 465 volumes** (`V_ATT1`, 73 distinct values — authored, not a template), **E4B 7.1 % of voices** once the factory cord amount is excluded (90.6 % carry the cord, 91 % of those at the template `+28`), **XPM 1.6 % of instruments**, KRZ unmeasurable (see below). It is also a large effect, not a nuance: of the AKAI keygroups that set it, **85 % change the attack by 2× or more**, median span 0.26.<br><br>**DONE: AKAI both directions**, on s3ked §47's five measured depths — the one envelope-scaling field on that machine that is both measured and alive, its five neighbours having been measured inert (and re-screened after they caught that the first detector could not have seen a timing change).<br><br>**OPEN: E4B** (cord `Vel<` → dest `0x49`; the destination id follows from the EOS manual's ordering plus our independently measured `0x4A` = VEnvDcy landing exactly on Dcy), **XPM** (`VelocityToVolumeAttack`, no measured law for what the MPC's 0..1 means in time), and **KRZ** — the K2000 has it natively on the `ENVCTL` page with VelTrk hard-wired to attack, but **`ENVCTL`'s byte offsets are not RE'd**: it is not a separate segment tag, so it sits inside `LYR` or one of the masked families and needs a disk-diff to locate. One K2000 constraint to carry into any write path: `ENVCTL` is per-LAYER and drives the amp envelope **and** ENV2/ENV3 together, so vel→ampEnvAttack and vel→filterEnvAttack cannot be set independently there. |
