...
| **AKAI: a volume we build fresh is a type-1 save, not a type-0** | The S3000XL's own SAVE offers two types, measured by s3ked 2026-08-18: **type 0 writes every file a volume holds** — programs and samples *plus* the effects file, the multi, the drum-input page and the take list — while **type 1 writes programs and samples only**. `build_akai_volume` emits programs and samples, so a volume we create from scratch is a type-1 save by construction and lacks all four aux files, which every real library volume carries. A user reloading it finds the effects and multi gone. Copying an existing image is unaffected — we round-trip all four byte-identically. **Reference specimen available:** the machine's own flash `BOOT SYSTEM#` is a type-0 volume written by the instrument (program, four waveforms, `TL1`, `EFFECTS FILE`, `DRUM INPUTS`, `MULTI FILE`), so a built volume can be diffed against what ENTIRE actually looks like rather than against a description of it. **Blocked on:** deciding what to write into `.X`/`.M3`/`.D`/`.T` — §AKAIAUX has the decode only half done, and an aux file with invented contents is worse than an absent one. §AKAIAUX |
| **KRZ: filter-table corrections — DONE, not hardware-confirmed** | Fixed 2026-08-17 from k2kremote's 40 panel anchors. **Codes 16 (HIPASS) and 73 (LP2RES) were unknown and refused** — a real filter dropped — now mapped to High1 and Low2. **Codes 5 and 17 are allpasses** and were mapped to XPM 23 (Model1 LP+dist), converting a filter that attenuates nothing into a distorted lowpass; now handled via `_K2_ALLPASS` → filter off, per Jan's call that XPM "off" is the true equivalent. Handled OUTSIDE the table on purpose: a table entry also reads `seg[1]` as a cutoff, and XPM type 0 maps to E4B 0x00, a 4-pole lowpass that is bypass-like only while wide open — so an allpass cornered at 300 Hz would have arrived as a real 300 Hz lowpass. Effect over the 721 corpus layers reaching the filter branch: **64 LP2RES and 11 HIPASS recovered, 17 allpasses corrected**. **Unheard on hardware.** §KRZF3 |
| **KRZ: F3 filters — DONE in the reader, NOT hardware-confirmed** | Re-landed 2026-08-17 (`6c37a1e`) with the gate the reverted attempt lacked. The four HOB segments are a fixed-size array — all four written for every layer of every algorithm, 1442 layers, 25 algorithms — so a 0x52 segment's presence says nothing about whether the slot exists, and its bytes are stale when it does not. `_ALG_DSP_FUNCTIONS` in `parsers/krz_parser.py` counts each algorithm's addressable Fn slots from the manual; 0x52 is read only at three or more. Agrees with 9/9 per-layer panel observations both ways. Corpus effect: **20 layers gain a filter, 48 refused**. **Still open:** the 20 recovered layers are unheard on hardware, and no slot→code table exists — `_K2_FILTER_TO_XPM` was measured against F1 alone (581/581), so an F3 code is decoded through a table with no authority there. A wrong *type* is still possible where a wrong *presence* no longer is. Resonance deliberately not read for F3. §KRZF3 |
