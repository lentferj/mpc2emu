...
| **`FILTER_ENV_FULL_CENTS` disagrees with `E4B_FENV_OCT_PER_UNIT` by 41%** | *(2026-08-22, §FENVFULLSCALE)* Two constants for one quantity — what a full FilterEnv→Filter-Freq cord is worth on the E4XT: **4383 ct (3.65 oct)** on the *input* path (`cents_to_filter_env_amount`, used by sfz/sf2) against **6168 ct (5.14 oct)** on the *output* path (`AKAI_ENV2_DEPTH_MAX`, `KRZ_FENV_FULL_CENTS`). They **compose rather than cancel**: an SFZ asking for 4383 ct reaches an AKAI or K2000 as 6168. E4B→anything is unaffected. **For eosed:** does §46 supersede MOD_DEPTH_CAL for this cord, or do the two measure different paths? Not changed unilaterally — it would move every sfz/sf2 filter-envelope depth by 41%. |
