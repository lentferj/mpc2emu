...
| **A drum program converts to nothing, and says so only in a log line** | `parse_xpm` skips a `type="Drum"` program (a pad is a one-shot hit, not a pitched zone) and returns a preset with **no voices and no samples**. It does not raise, so a caller cannot tell "nothing to convert" from "converted" — same for MIDI/Plugin/Audio/CV/Clip programs, which reference no sample data at all. **Not a marginal file type:** in a real 571-file MPC One backup the drum programs are 90 files holding **940 sample references, a median of 12 each** — *more* sampled material than the 82 keygroup programs carry (964 refs, median 5, and 25 of those hold 0 or 1 sample). Wanted, in order of value: (1) build a preset from a drum program's pads, one pad per key — the machinery exists, since `--from-samples` already maps one-shots onto consecutive keys and since 2026-08-02 spreads root-note collisions, which is exactly a drum kit's shape; (2) failing that, refuse a non-keygroup program with a clear error instead of returning an empty preset. Raised from VinSamLib 2026-08-03, which greys such rows out rather than offering an import that silently yields nothing |
...
| **XPM loop crossfade and `loopFineTune` are dropped** | `loopCrossfadeLength` (and its `sliceInfo` twin) and `loopFineTune` are parsed nowhere, so a crossfaded MPC loop converts as a hard splice and can click. CWM turns the crossfade into a fraction of loop length. Checklist B5; §XPMGAPS |
| **XPM `ZonePlay` / play logic is dropped** | the per-instrument zone-selection mode (round-robin / random / velocity) is read by CWM and ignored by us, so a round-robin keygroup collapses to one fixed choice. §XPMGAPS |
