...
| **AKAI: converted volumes start at PRGNUM 0 and stack with the machine's boot program** | `build_akai_volume` numbers programs sequentially from 0 per volume and writes `PMCHAN 255` (OMNI), both of which are correct in isolation. But the S3000XL's `TEST PROGRAM` **survives every CLR** and sits on PRGNUM 0, so a user loading a converted volume gets both answering the same program change — and an OMNI program answers on every channel, so choosing a different one does not escape it. Confirmed on hardware 2026-08-18 during the release test, where the boot program had to be moved to PRGNUM 100 before a clean capture was possible. **Ours, not the machine's.** Options: start user volumes at PRGNUM 1, or make the base number an option. Needs a decision on which is less surprising, not more RE. §AKAIRELTEST |
