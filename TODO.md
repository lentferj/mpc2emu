...
| ~~**`e4b_parser` reports every release ~3.9x too long**~~ **FIXED for sustain > 0, still open at sustain 0** | *(2026-09-18)* `env_release = env_rate_to_span_seconds(_rel_span, pzt[8]) + _stage2_seconds(pzt[10])` asks how long segment 1's RATE would take to cross the WHOLE release span -- but the writer stops that segment at the knee. The over-count is `_rel_span / _r_mid_db` = 97.82/29 = **3.37x** on the first term, plus segment 2's time, giving the ~3.9x observed. Verified on a purpose-built ladder: 0.25/0.5/1/2/4 s written, read back as 0.971/1.919/3.974/7.854/15.686 -- a constant factor across a 16x range.<br><br>**WHICH HALF IS WRONG IS NOT YET PROVEN.** eosed's hardware read-back confirms the WRITER agrees with the MACHINE, which rules out writer-vs-machine disagreement and says nothing about writer-vs-SOURCE -- if the reader is right and the writer is 3.9x fast, the machine would agree with the writer exactly as observed. My inference that the reader was the wrong half was invalid and is withdrawn. **The discriminator is the MPC measurement above**, and on its evidence the writer is too SLOW (we reach 29 dB at 0.96R where the MPC reaches 60 dB at 0.95R), which points at the reader -- but that is an argument, not the arithmetic. Fifth writer/reader inverse-pair defect found today.<br><br>**FIXED 2026-09-18 for the case it could be verified in.** Release segment 1 stops at the KNEE, so its span is sustain->knee, not the whole release span -- the same error the attack block in this file records being fixed on 2026-09-11, one stage over: *true of the stage as a whole and false of each SEGMENT, which is the unit the rate bytes act on.* The ladder now reads back **1.93-1.97x** instead of 3.9x, and that residual is the WRITER's real geometry (0.96R for segment 1 plus R for segment 2 = 1.96R), i.e. the shape defect recorded above, not a reader error.<br><br>**STILL OPEN AT SUSTAIN 0, and deliberately left as it was.** There the writer clamps the knee level to silence (`_env_db_to_level_byte(decay_span + 29)` with `decay_span` already the full span), so where the knee sat is **not recoverable from the file**. Assuming the full span reproduces the original over-count; the previous behaviour is kept -- not because it is right but because it does not over-count. **The real fix is writer-side:** encode the knee so it survives, or accept that sustain-0 releases do not round-trip. Affects percussive material, which is most sustain-0 material.<br><br>**Which half was wrong was settled by the SOURCE.** The MPC reaches 29 dB down at 0.86R; this writer reaches it at 0.96R -- slightly slow, nowhere near 3.9x fast. An earlier argument that the writer must be right because the machine agreed with it was invalid (it rules out writer-vs-machine disagreement and says nothing about writer-vs-source), and eosed caught it.<br><br>**Also open, from eosed:** they recalibrated the E4XT release byte -> dB/s law over bytes **44-86** and the version in our tree is up to **14 % out at byte 44 with the curvature sign inverted** (+0.000175 b^2 fitted over 58-86, -0.0000889 over 44-86). Both laws agree in 53-64 where tonight's work landed, so nothing measured today is affected -- but `e4b_writer` inverts that law to pick bytes, so it matters wherever a release is short. Their fit: `ln(dB/s) = -0.0000889 b^2 - 0.045224 b + 6.8949`, r2 0.99809. Not adopted; residuals not seen here. |
| **E4B release tail collapsed 23x too fast -- FIXED 2026-09-18, not yet heard** | The two release segments were SHARING the release time: segment 2 got `1 - _ENV_SHAPE_BREAK_TIME` = **4 % of the duration for two thirds of the fall**, about 590 dB/s. On a sustaining pad the note tracked the source down to -20 dB then vanished in 0.02 s where the source takes 0.50 s.<br><br>**Fixed with no new constant:** each segment now takes the whole release time for its own span. `_ENV_SHAPE_BREAK_TIME` governs segment 1 only.<br><br>**Both sides measured independently and converged.** Source (20 s hold, sustain ripple 0.71 dB, two reps agreeing to 0.02 s): first 20 dB **9.78 dB/s**, -20 to -60 **25.24 dB/s** -- the MPC's release is CONVEX, which `xpm_parser` §MPCENVREL recorded before it was measurable and which this rediscovered the hard way. eosed drove the E4XT directly and measured **25.40 dB/s against 25.24, r2 0.9984**. The writer now selects byte 72 against eosed's measured best of 71 -- one byte, 6 % in rate.<br><br>**THE KNEE STAYS AT 29.0, TESTED NOT ASSUMED.** Shallowing it toward the MPC's own -20 dB break is WORSE: mean error 9.2 % against 7.8 %, below -30 dB 7.2 % against 2.8 %. A shallower knee starts the fast segment sooner and the whole tail arrives early; depth and the two rates are a matched set.<br><br>**Open:** not heard by Jan, and the convexity ratio rests on ONE sustaining pad -- the same exposure that made the K2000 factor right on its calibration program and 2.6x wrong on the next. A second sustaining program with a different release length is the test. **Also open: `_ENV_SHAPE_MAX_FRAC` and the decay path** use the same share-the-time assumption at `_dcy`/`_dcy2` and have NOT been examined.<br><br>**Method note that cost two hypotheses:** at a 4 s hold this pad has not settled -- it swells for ~10 s, the reference drifts 13 dB and rep spread is 0.38-0.53 s. That produced a fake S-curve, a fake 'MPC is modulating and we drop its LFO' theory, and a withdrawn 0.5 % agreement. At 20 s: ripple 0.71 dB, spread 0.02 s. **A dispersion figure without its window is not a number.** |
| **ALL THREE TARGETS ARE WRONG ON SUSTAINING PROGRAMS -- each in a different direction, and every one was verified at sustain 0** | *(2026-09-18, measured after Jan asked "what about the other conversion targets? Do they need reevaluation?")* **Yes, and it is systematic.** One metric, one session, same source WAVs on every machine: seconds from note-off to fall 30 dB below the note-off level.<br><br>&nbsp;&nbsp;`target                 sus 0.00   sus 0.63   sus 1.00`<br>&nbsp;&nbsp;`KRZ (before tonight)       1.04x          2.19x         2.60x`<br>&nbsp;&nbsp;`KRZ (as shipped now)       1.04x          0.91x         1.08x`<br>&nbsp;&nbsp;`AKAI                       1.00x          0.90x         1.47x`<br>&nbsp;&nbsp;`E4B                          --             --          0.75x`<br><br>**THE COMMON CAUSE IS STRUCTURAL, NOT THREE SEPARATE BUGS.** Every writer computes its release span as a function of SUSTAIN, and every one was calibrated and ear-verified on a sustain-0 program -- where all three are right. `krz_writer` had two knee branches sharing one constant; `akai_s3000_writer` has `span_rel_db = _AK_SUSTAIN_DB_PER_UNIT * sus` with the caveat already written beside it (*"the right span ONLY if the key is held until the decay has finished"*); `e4b_writer` has `rel_span = _ENV_FULL_SPAN_DB - decay_span` and a knee depth `min(_ENV_SHAPE_KNEE_DB, _ENV_SHAPE_MAX_FRAC * rel_span)`, both sustain-dependent and its own comment says *"INFERRED from the segment model ... NOT hardware-confirmed"*.<br><br>**E4B ALSO HAS A SHAPE FAILURE, not just a scale one.** the pad: -20 dB at 2.45 s, **-30 dB at 2.47 s -- the last 10 dB in 0.02 s**, against the MPC's 0.50 s. The tail collapses **25x** too fast, which is the two-segment knee dumping the remainder in its second leg. A scale factor will not fix that; the knee itself is in the wrong place.<br><br>**THIS REOPENS A DECISION MADE ON A WRONG EXPLANATION.** The row below on the E4B release knee declined a retune partly because *"the sustaining synth is fine"* while the pad was long, and explained the pad as *"the most exposed case the format has ... not representative"*. The real discriminator is **sustain** (1.00 against 0.63), not exposure -- the same split that governs all three targets. The duration-weighted-metric point in that row still stands; the reason for declining does not.<br><br>**Status: OPEN, nothing changed for AKAI or E4B.** KRZ is fixed and on the card. The fix for the other two is the same exercise as tonight's KRZ work -- calibrate the span against two or three sustain values with the sibling sessions driving the parameter live -- but the AKAI's error is much milder (1.47x against 2.6x) and the E4B's needs the knee located before any factor is touched. **References already measured and reusable:** MPC the pad 2.38/2.80/3.30 s and the MPC sustaining synth 0.30/0.46/0.69 s at -10/-20/-30 dB, MPC on its own port, MIDI channel 12, audio capture 5/6 (Jan must arm the track each time). AKAI sustaining synth ch15, the pad ch16. E4XT ch5, P003. |
| **KRZ release: TWO factors, one per envelope branch -- the single-factor version shipped and was wrong** | *(2026-09-18)* Jan: *"the release on the KRZ sustain-0 synth is too short compared to all 3 other versions"*. **Two defects, both fixed.**<br><br>**(1) The 80/20 split ate 20 % of every below-knee release.** When sustain sits under the 33 % knee, `_fill_env` aims BOTH release legs at silence, so Rel1 reaches 0 on its own and Rel2 never sounds -- yet Rel1 got only `rel * 0.8`. Predicted from Rel1 alone 99.37/0.840 = 118.3 dB/s against 122.3 measured (3.3 %). Rel1 now carries the whole release. k2kremote supplied the correction that the machine stores **840 ms, not the 832 the writer computed** -- the K2000's envelope times are a **quantised ROM table** (0x1FBA04), not a continuum.<br><br>**(2) `KRZ_RELEASE_FACTOR` 1.9 -> 3.65.** Measured with one metric across all three machines (seconds from note-off to fall 30 dB below the note-off level): MPC **0.420 s**, AKAI **0.420 s**, K2000 old **0.225 s** (1.87x fast), K2000 at Rel1 2.000 **0.410-0.435 s** over three captures. k2kremote drove Rel1 over SysEx live while this session captured, so no rebuild was needed to walk the curve. Jan by ear on 304: *"sounds pretty good and close"*.<br><br>**STATUS: OPEN, because the factor is a calibration and not a law.** The arithmetic asks for 1.420 s and the machine wants 2.03 -- a **1.43x that is not explained**. Ruled out on hardware: Dec1 (tripled it, the fall moved 3.7 %) and the K2000's own release (k2kremote: clean and linear on a decay-free subject, +0.066 dB/s intercept against slews of 20-200). The remaining candidate is the samples' own decay, and `MPC_RELEASE_SPAN_DB = 38.3` is contaminated by the SAME samples -- two contaminated numbers on opposite sides give a factor that looks like a machine constant and is not one.<br><br>**THE EXTRAPOLATION IS THE EXPOSURE.** Calibrated at a 547 ms source release; the same bank carries the pad at 1.20 s (Rel1 4.38) and the sustaining synth at 2.85 s (**Rel1 10.40 s**). The three K2000 points do not scale proportionally (0.840->0.225, 1.420->0.295, 2.000->0.410), so 3.65 at 5x the calibration point is not supported by anything measured. **Those two programs are the test** -- they ship in the rebuild and Jan hears them.<br><br>**What would close this:** re-measure the MPC's release span the way k2kremote measured the K2000's -- decay-free subject, several release settings, regress slew against 1/T, check the intercept. That makes `MPC_RELEASE_SPAN_DB` the envelope's span rather than this program's and should remove the need for a factor at all. Needs MPC bench time with a suitable program (Jan has to arm the track; MPC is on its own port, **MIDI channel 12**, audio capture 5/6).<br><br>**UPDATE, same evening: the 3.65 shipped to the card and was WRONG for every sustaining program.** Jan heard the sustain-0 synth right and the pad would not stop. `_fill_env` has two branches and they map Rel1 to audible fall **4.7x differently**: below the knee both legs aim at silence and Rel1 carries the whole fall (ratio 0.22); above it Rel1 only fades to the 33 % knee and Rel2 finishes (ratio 1.03). One factor served both, so fixing the sustain-0 case made every sustaining program **2.6x too slow** -- a 10.4 s tail on a pad.<br><br>**The old 1.9 was one branch's law used for both.** It was fitted on AlphaPad #200, a SUSTAINING program: roughly right above the knee, badly wrong below it -- exactly the case Jan reported. Same category error k2kremote found in their own `99.80 dB` span, measured on a sustaining subject and applied everywhere.<br><br>**`KRZ_RELEASE_FACTOR_SUSTAINED = 1.38` added for the above-knee branch**, below-knee stays 3.65. Measured at TWO Rel1 settings per program (k2kremote driving SysEx, this session capturing), which is what made it an interpolation rather than the extrapolation that failed: a single pair per program implied a proportional law and 1.55, and at 1.55 **both** came back slow. The law is affine -- `pad t(-30) = 0.969*Rel1 + 0.520`, `sustaining synth 0.964*Rel1 + 0.112`. **Slopes agree to 0.5 % (the machine), intercepts do not (the program)**, and each intercept is close to a QUARTER OF THAT PROGRAM'S OWN Rel2 leg (2.08 s and 0.36 s -> 0.52 and 0.09 against 0.520 and 0.112).<br><br>**Result, full fall against the MPC:** pad 1.06/1.15/1.08x at -10/-20/-30 dB, sustaining synth 1.10/1.15/0.91x. Within 6-15 %, from 2.6x. The sustaining synth crossing below 1.0 at -30 dB is a SHAPE difference (two-leg approximation vs the MPC's convex fall), not scale.<br><br>**Rel2 CONTROLLED TEST RUN 2026-09-18 evening -- mechanism confirmed, coefficient refuted.** Rather than the third program both sessions had assumed was needed (which would have been one more observation differing in release time, sustain, samples AND Rel2 at once), k2kremote varied **Rel2 alone** on 303, 2080 -> 500 ms, everything else re-read and proven unchanged. Two Rel1 points before and after.<br><br>&nbsp;&nbsp;`estimator     slope before -> after      intercept before -> after`<br>&nbsp;&nbsp;`-10 dB        0.804 -> 0.800             -0.021 -> +0.013`<br>&nbsp;&nbsp;`-20 dB        0.975 -> 0.982             +0.157 -> +0.005`<br>&nbsp;&nbsp;`-30 dB        0.969 -> 0.984             +0.520 -> +0.069`<br><br>**The SLOPE is invariant under a 4.2x change in Rel2** -- the machine's law, now established by manipulation and not by two programs agreeing. **The intercept is set by Rel2** and collapses toward zero. **The -10 dB row is the control**: a fall that never reaches the knee should not care what Rel2 is, and it moved 0.02 s both times. Without it every other number is equally consistent with Rel2 changing something unrelated.<br><br>**`intercept = Rel2/4` IS REFUTED, and it was an artefact of the measurement depth.** Fitting the baseline on three estimators instead of one shows the intercept GROWS with depth (-0.021 / +0.157 / +0.520), so the quarter held only at -30 dB -- the depth I happened to pick, on the thinnest (two-point) fit. Proportionality then predicted 0.125 after the change and measured 0.069. **So: Rel2 sets the intercept, monotonically with depth, but no coefficient is established.** Declaring the estimator before looking is what caught this; k2kremote asked for it.<br><br>**COEFFICIENT PINNED 2026-09-18 (Jan: "pin the coefficient"), and it produced a model with a passing control rather than just a number.** Five Rel2 levels (500/1000/1500/2080/3000 ms, all exact table entries, no quantisation anywhere in the series) at a **FIXED Rel1 of 8300 ms** -- fixing Rel1 makes `slope * Rel1` common to every reading, so a difference between two readings IS a difference in intercept, with no line fitted and no slope assumed. That removed the two-point-exact-fit weakness Jan challenged (*"two measures are enough to settle this?"*).<br><br>&nbsp;&nbsp;`Rel2    t(-30)    predicted    miss       role`<br>&nbsp;&nbsp;`  500    8.235      8.235      --         fit point`<br>&nbsp;&nbsp;` 1000    8.330      8.344      -0.014     OUT OF SAMPLE`<br>&nbsp;&nbsp;` 1500    8.460      8.453      +0.007     OUT OF SAMPLE`<br>&nbsp;&nbsp;` 2080    8.580      8.580      --         fit point`<br>&nbsp;&nbsp;` 3000    8.770      8.781      -0.011     OUT OF SAMPLE`<br><br>**Proportional across a 6x range.** Three genuinely out-of-sample points, misses straddling zero, max 0.014 s against a noise sd of **0.005 s** (4 repeats). k2kremote's saturation mechanism is REFUTED -- they wrote before the capture that if 3000 landed on the line the honest summary was that their mechanism is wrong, not that it saturates above 3000.<br><br>**THE MODEL, and the control is the point.** Rel2's coefficient varies with measurement depth: **+0.2165** at -30 dB, **+0.0689** at -20, **-0.0121** at -10. Solving `coeff = (drop - knee_dB)/span` from the -20 and -30 rows gives **knee at 15.3 dB below peak, Rel2 spanning 67.8 dB**. The **-10 dB row was not used to fit it**: it sits ABOVE that knee, so Rel2 should not reach it at all, predicted coefficient 0 -- measured -0.012, flat across the whole 6x range. An out-of-sample control on a parameter the fit never saw.<br><br>**THIS CONTRADICTS THE CODE.** `_REL_KNEE_PCT = 33.0` in `krz_writer` is commented *"Rel1 target, DISPLAYED percent (~-24.8 dB, measured)"*. The measurement above puts the knee at **-15.3 dB**, a 9.5 dB disagreement. Either `krz_db_to_level_pct` mismaps 33 %, or the -24.8 figure is right about the panel and wrong about what the engine does. **Not acted on** -- it is derived under an assumption of dB-linearity within a segment, and it deserves a direct check (set Rel1's TARGET LEVEL and find where the fall changes slope) before any constant moves. Affects the two-leg shape on every sustaining KRZ program.<br><br>**PRACTICAL LEAD, not chased:** with Rel2 short the intercept is ~0 and `t = 0.98 * Rel1` becomes a clean proportional law -- and the -30 dB match against the MPC improved to 0.96x from 1.08x. `_REL1_TIME_FRAC = 0.8` may simply be the wrong split for matching this source. Worth a look before building the solve-against-the-law writer below.<br><br>**THE RESIDUAL CAN BE REMOVED, NOT JUST AVERAGED, and that is the next step.** Rel2's length is known at write time, so the writer could solve Rel1 against the affine law instead of scaling a total. **Needs a third program** to confirm the quarter-of-Rel2 relation before anything is built on it -- two points that fit a rule are not the rule.<br><br>**Also explains `MPC_RELEASE_SPAN_DB = 38.3`**, which this row previously recorded as an unexplained 1.43x. It was measured on the sustain-0 program, where the samples' own decay is mixed into the fall. The two sustaining programs imply **21.6 and 25.9 dB**. The constant is not wrong; its stated scope is.<br><br>**A REGRESSION THIS NEARLY SHIPPED, recorded because the reasoning was the attractive kind.** Making `xpm_parser` carry `release_rate_db_per_s` for MPC sources is more principled and is what `krz_writer`'s own comment asks for -- but all three writers PREFER that field, and it moves the AKAI's RELSE1 from **55 to 59**, where 55 is hardware-confirmed and ear-approved. The AKAI's seconds path is empirically calibrated against MPC material and matches the source exactly. Backed out. **A derivation that is more principled than an empirical fit is still worse when the fit was made against the real thing.** |
| ~~**Every KRZ conversion played ~14 dB below the K2000's own programs**~~ **FIXED 2026-09-18, awaiting a re-listen** | Jan, comparing at identical K2000 output gain and identical AD input gain: *"the output levels ... are significantly lower than on the other samplers or the MPC"*. **Measured deficit 14.4 dB** against ROM 199 with the F4-AMP block byte-identical, and Jan raising `Adjust` on the panel by +12 dB recovered **12.2 dB**, landing 1.3 dB from the machine's own program.<br><br>**Root cause: a compensation for gain we never added.** `_TPL_LAYER` is copied from a ROM program and carries 12 dB of program-scope level (`Adjust` +6 at [1], wire `Gain` byte 4 at [13], which is a DESCENDING six-step enum, `dB = (5 - byte) * 6`, so 6 dB). `KRZ_PROGRAM_BASELINE_DB` subtracted that 12 dB from every sample's `volumeAdjust` as though the writer had *added* it. It had not — the donor template brings it, and ROM 199's own samples sit at `volumeAdjust 0.0 dB`. So we shipped every sample 12 dB pre-attenuated into a program whose gain the machine already treats as normal.<br><br>**THE READER IS THE OTHER HALF, and is why zeroing the writer alone is wrong.** `krz_parser` reads that same program-scope level and adds it into zone volume, so writer and reader were an exact inverse *pair* — 12 dB out, 12 dB back. Zeroing only the writer makes every KRZ→KRZ round trip 12 dB louder than the last; `test_krz_writer.py::test_sample_gain` and `test_krz_roundtrip.py::test_geometry_roundtrip` both failed and were the only signal. Fixed by giving the reader the matching reference: `KRZ_HOUSE_PROGRAM_GAIN_DB = 12.0`, subtracted so `program_gain_db` reports gain **relative to the K2000's house level** rather than absolutely. A stock program now reads 0 dB of boost; a genuinely hot one reads only the part that is really extra.<br><br>**Independent control, measured 2026-09-18 17:10** on two third-party banks Jan loaded for the purpose — 700 block built on ROM samples, 800 block on its own samples, one pass, one gain staging, onsets detected from the envelope rather than the MIDI schedule. Peak medians: ROM programs **-19.2 dBFS**, 700 block **-25.0**, 800 block **-20.9**. **Own-sample banks are not systematically quieter than ROM-sample banks**, and both sit within a few dB of the ROM programs — so 12 dB is where real K2000 material lives, and the 14.4 dB hole was ours, not the machine's.<br><br>**Guarded by a test, not a comment** (`tests/test_krz_house_gain.py`): the template's two raw bytes are asserted to equal the constant the reader subtracts, and both output wires to carry the same `Gain`. Nothing in the format ties those together, and a round trip cannot catch a drift in either — a changed template would simply move both halves together and stay green.<br><br>**Still open:** the rebuilt banks have not been heard. Blocked on the source XPM paths and build options (asked VinSamLib 2026-09-18). The K2000 card is in the PC. |

| **KRZ conversions not rebuilt with the measured pan calibration** | *(2026-09-18, Jan: "postpone krz, record it as open")* The K2000 LFO->pan depth was measured and wired the same day (`KRZ_LFO_PAN_DEPTH_SCALE = 0.4377`, dc7ba89): **0.372 dB/byte, constant to +/-0.003 over a 10x range of the parameter** — the most linear of the three targets — putting the MPC's 5.11 dB reference at byte 14 where the writer had been emitting byte 32.<br><br>**UNBLOCKED 2026-09-18 and commissioned.** Jan heard the E4B and AKAI rebuilds ("all sound good") and asked for the same on KRZ. The hold was never technical — it was that rebuilding all three at once would put three unheard changes in front of one listener, which is how a wrong one gets blamed on the wrong machine. Two are now judged, so the third can go.<br><br>**KRZ inherits more than the pan fix.** The `SliceEnd`/placeholder-loop truncation (§XPMSLICEVETO) is reader-side, so every KRZ conversion ever made carried it too — on this material that is 58% of one program's audio. The AKAI release and velocity-filter fixes are writer-specific and do NOT apply here; the K2000 has its own envelope path and has not been checked for the same class of fault.<br><br>**When it is picked up:** the sweep and its caveats are at `KRZ_LFO_PAN_DEPTH_SCALE`, and the thing to re-read first is §K2PANWIRES — a K2000 PANNER doubles its wire and does NOT itself pan, so a program whose OUTPUT wires are left centred sums them and is silent however hard the panner is driven. Our writer spreads them (`0x70` / `0x90`, opposite signs: `0x90`/`0x94` are BOTH pan -7 and sum exactly like centred wires), so converted programs are unaffected — but any hand-built comparison program needs the same two bytes or it will measure zero and look like a broken conversion. |
...
| ~~**Every MPC tremolo is silently dropped on the E4B path**~~ **FIXED 2026-09-17** | *(2026-09-14, found auditing constant provenance; reopened 2026-09-17 after a wrong closure; closed by measurement the same day.)* The E4B writer emitted **no LFO→AmpVol cord at all** — `_extra_cords` carried LFO1/2 to Pitch, Filter-Freq, Filter-Q and AmpPan and stopped there — and `xpm_parser` read `LfoVolume` **zero times**, so the routing was lost twice over. `E4B_LFO_VOLUME_FULL_DB` was 24.0, marked UNMEASURED, and **referenced by nothing in any conversion path**: a constant can be wired to nothing and still look maintained, which is why a provenance audit found this and three months of envelope work did not.<br><br>**Found by Jan at the MPC, not by any test here**: `the pad` keygroup 4 shows LFO Tri 0.20 Hz with DESTINATIONS PITCH 0, FILTER 0, **AMP 29, PAN 81** — the pan arrived in the conversion and the amp did not.<br><br>**What was measured** (MPC One, Square at 0.1 Hz, AMP swept by hand, Scarlett pre-EQ tap): the swing is bipolar but **not centred on the un-modulated level**. In amplitude, `(1 − d/2) + d·lfo` with `d = AMP/127` — the peak rises by `d/2`, the trough falls by `1.5d`, and **the centre sinks by `d/2`**, reaching actual silence at AMP 85. Predicted 5.15 / 14.20 / 27.55 dB at AMP 32 / 64 / 80 against 5.16 / 14.20 / **27.58** measured, the last predicted from the other two.<br><br>**What was built.** `lfo1_to_volume` carries the swing (halved to the model's one-sided unit, which was established from the K2000 and AKAI readers rather than assumed) and a new `lfo_volume_centre_db` carries the sink. The E4B writer emits **two cords into AmpVol**: `Lfo1~` (0x60) for the swing and `DC` (0xA0) at a negative amount for the centre. `E4B_LFO_VOLUME_FULL_DB` is now **94.78** and read — a least-squares slope through the origin over eosed's two sine rungs, reconciled with the independent DC sweep by reading both as **dB per PERCENT** of panel amount (±127 byte = ±100 percent), where they agree to 1.7 % instead of disagreeing by a factor of two. 12 tests in `tests/test_xpm_tremolo.py`, with the centre asserted separately from the swing so a swing-only implementation fails, and negative controls at the solver, the parser and the writer.<br><br>**The cost, stated rather than buried:** two knobs cannot place three quantities, so the top overshoots — 0.23 dB at the corpus median, 3.72 at p95, 16.31 at the deepest of 26,378 non-zero keygroups. Past the measured 8 dB ceiling the **swing** gives way and the centre does not, so on the 2.41 % of keygroups asking `d ≥ 2/3` the trough stays audible where the original goes silent. Full reasoning in `docs/RESOLUTION_NOTES.md` §MPCLFOAMP.<br><br>**HEARD AND CONFIRMED 2026-09-18** (Jan, on the rebuilt one-bank disc): *"emu is fine now - I think those are very good conversion results"*. First time this tremolo reached a listener — it was measured, tested, round-tripped and written to two discs beforehand. The confirmation covers the swing, the centre sink and the tremolo/pan phase relationship, which was the one judgement no measurement here could settle. **Still open:** the MPC→AKAI and MPC→KRZ write sides do not carry the centre, though both machines have a level control that could. |

| **Four more source fields are silently dropped — found by audit, not by noticing** | *(2026-09-14, `tests/test_source_fields_unread.py`)* eosed asked whether the dropped-tremolo class was mechanically findable. It is, by a DIFFERENTIAL test: change a field in a real source file, re-parse, see whether anything in the Bank moves. **95 of 132 fields cannot change our output.** Ranked by how much real material actually uses them: **`OneShot` 5,828 instruments (7.1%)** — sample playback mode, changes note-off behaviour; **`VelocityToFilterEnvelope` 3,960 (4.0%)**, 23 distinct values so it is genuinely dialled; **`MuteGroup` 1,693 (1.7%)** — the model has no field, yet `krz_writer` writes mute groups and the AKAI keygroup carries one, so we can express it on two outputs and cannot read it from the source that most often states it; **`FilterADEnvelope` 1,484 / `VolumeADEnvelope` 1,243** — if these select an attack-decay envelope with no sustain stage then the MODAL value is the one we are not modelling and ~98,000 instruments use the wrong envelope model, **a documentation question before it is a measurement one**. **CORRECTED 2026-09-14, and the correction is the point:** the first version of this row led with the PITCH ENVELOPE at "7,238 instruments carrying non-default values". That figure counted values differing from the modal STRING. `PitchEnvAmount` has exactly two values in the whole corpus, **0.5 and 0.5001 — both centre, i.e. no pitch modulation anywhere**, and the two profiles are two factory defaults. The field is genuinely unread and all three targets could carry it (K2000 `ENV3` is a dedicated pitch envelope, the E4XT has an aux envelope plus Pitch cord destination 0x30, the S3000XL's matrix has Env2→pitch in the factory routing we already write at zero amount) — but **no corpus material uses it, so it ranks last rather than first**. Ranking on "differs from the modal value" instead of "differs audibly" is the same defect class this audit exists to catch, committed inside the audit itself. **Blocked on** deciding which to implement first; all are reader-side. |

| **Modwheel → LFO/mod depth exists on ONE of eight conversion paths** | *(2026-09-14, Jan's priority)* The model carries `wheel_to_lfo`; only MPC→E4XT implements it, splitting each LFO cord into a static `D*(1-Kw)` plus a `ModWheel → CordN-Amount` cord of `D*Kw`. **Most of the RE is now done and all three target idioms are hardware-measured.** **E4B**: source ids confirmed (`ModWl 0x11`, `MidiA 0x14`; the id block is banked in eights so the manual's list order cannot be counted through — `0x16/0x17` are foot SWITCHES where a naive count puts MIDI C/D). Cord-amount destination is `168 + n` for ANY n, not just 0-3. Factory CC block from firmware: MIDI A-H = CC 21-28, EOS reaches CC 0-31, no RPN/NRPN handling on CC 6. **KRZ**: `Src2 = LFO1, MinDpt = D*(1-Kw), MaxDpt = D, DptCtl = MWheel` maps Kw with NO approximation — k2kremote §73 measured `DptCtl` as SCALING linearly between Min and Max (wheel 0/64/127 -> +0.99/+1.97/+3.00 st), with `Dpt` provably not in that wire's arithmetic (changing it 500ct->0, everything else held, moved nothing). **AKAI**: the assignable matrix is the WRONG mechanism — there is a dedicated `MWLDEP` at program offset 36, unsigned 0..99, and **our writer already emits the factory 30 there** (`akai_s3000_writer.py:2414`). Corpus: `MWLDEP` non-zero on 213 of 213 programs, 202 at exactly 30; the matrix route is used once. The modwheel is UNIPOLAR on both destinations (s3ked §249, §251), so polarity is a source property. **REMAINING, one question per format:** (a) does `MWLDEP` SCALE `LFODEP` or ADD to it — decides whether `wheel_to_lfo = MWLDEP/99`; the s3ked §251 sensitivity does NOT transfer, it was measured on the signed ±50 `MODVLVOL`, a different path. (b) do KRZ `MinDpt`/`MaxDpt` use the same nonlinear byte→cents curve as `Dpt` — §KRZLFOPITCH read this exact field through the wrong scale once, endpoint looked right, small vibratos came out 20x too shallow. Both discriminate on a wheel-down rung against a baseline. **Also still missing on the AKAI: we write a FIXED 30 rather than the source's Kw, and never read offset 36 at all.** See `docs/RESOLUTION_NOTES.md` §MODWHEEL, which also carries Jan's data-entry rule. |

| ~~**KRZ: the PITCH page's Coarse transposition is never read**~~ **FIXED 2026-09-14, pending an end-to-end listen** | `krz_parser` read `cur.transpose = CAL[1]` and dropped the PITCH page's separate `Coarse`. k2kremote confirmed by panel read that **`CAL[1]` is the keymap `Xpose`** (probe 7 into offset 178 reads `7ST`), so our existing read was correct and nothing written back is wrong — **the fix is additive**. They then measured the arithmetic acoustically against a (0,0) reference so the sample's own tuning cancels: Xpose 12/Coarse 0 -> +11.97 st, Xpose 0/Coarse 12 -> +11.97, **Xpose 12/Coarse 12 -> +23.98** (additive, not override), 7/0 -> +6.99, 0/7 -> +7.04. The two single-field rungs are the carried known. **Now `krz_total_transpose(cal) = Xpose + (CAL[17] − CAL[15])`**, the Coarse half read as an 8-bit signed DIFFERENCE — `CAL[17]` alone is wrong wherever `CAL[15]` is set, 5.7% of layers. **Impact: 30.1% of 43,424 corpus layers decode differently, 16.4% by ≥12 semitones, and 22.4% had Xpose at zero so the whole transposition was lost.** Regression test `tests/test_krz_transpose_coarse.py` carries the six measured rungs and k2kremote's six-point encoding demo; **negative control run — 5 of 8 fail with the fix reverted.** The test calls `krz_total_transpose` rather than reimplementing it, after the first draft copied the arithmetic and would have passed with the parser reverted. **The suite passed unchanged at 768 because the single in-repo KRZ fixture has Coarse = 0 on all 22 of its CAL segments** — the path was entirely untested, which is how it survived. **Open:** confirm by ear on the K2000R that a Coarse-using program now converts at the right pitch. See §KRZCOARSE. |

| ~~**KRZ: the PITCH page's Src2 wire is never read**~~ **FIXED 2026-09-15 (read AND write), pending an end-to-end listen** | `krz_parser` read `Src1`/`Dpt` and nothing else on the pitch page. **Corpus: LFO1 sits on Src1 on 4.6% of 43,424 layers and on Src2 on 22.6%** — 21.8% on Src2 and not Src1, 21.4% with a live depth, their vibrato dropped entirely. Src2 also carried `MPress` on 6,218 layers and LFO2 on 1,743. **Semantics all HW-measured (k2kremote §72-§74):** `DptCtl` (cal[23]) SCALES linearly between `MinDpt` (cal[24]) and `MaxDpt` (cal[25]) — wheel 0/64/127 gave +0.99/+1.97/+3.00 st for Min=100 ct Max=300 ct; **with `DptCtl` OFF the wire sits at `MinDpt`, not `MaxDpt`** (14.2% of layers are DptCtl-off with Min≠Max, so reading MaxDpt would overstate every one); `Dpt` is provably not in that wire's arithmetic (500 ct→0 with everything else held moved nothing); and **all three depth fields share `krz_lfo_pitch_byte_to_cents`**, panel-confirmed at ten bytes (k2kremote §74) including six below 100 cents — the region where §KRZLFOPITCH's error hid while its endpoint looked right. **Implemented as `krz_pitch_lfo_src2(cal)`** returning (depth, wheel-gate), summed with the Src1 wire because two wires into one destination add on the machine. **The writer emits the gated form too**: `Src2 = LFO1, MinDpt = D*(1-Kw), MaxDpt = D, DptCtl = MWheel` when `wheel_to_lfo > 0`, and the unchanged Src1 path when it is zero, so every pre-existing conversion stays byte-identical. **That completes KRZ↔X for the modwheel feature in both directions.** 23 tests in `tests/test_krz_pitch_src2.py`; **negative control run — 4 fail with the wire made unread again.** Round-trip tolerances are the byte grid's own: swept over 793 (depth, Kw) pairs the worst errors are 0.0294 on depth and 0.0667 on the gate — a first pass on four hand-picked values showed the gate round-tripping to 0.0000 because the quantisation of Min and Max cancels in their ratio at those points, which reads as precision and is coincidence. **Open:** confirm by ear on the K2000R. |

| **14 citations of the form `RESOLUTION_NOTES §N` name a file all four projects have** | *(2026-09-15, found closing the citation thread with s3ked)* **This project has never numbered a section** — all 299 headings in `docs/RESOLUTION_NOTES.md` are named (`§KRZCOARSE`, `§MODWHEEL`), verified back past 2026-08-20. **So every `RESOLUTION_NOTES §<number>` in this tree refers to a sibling's file, and all four projects have a file of that name.** s3ked's rule for it: *naming the file does not disambiguate when the filename is shared* — `RESOLUTION_NOTES §6` looks fully qualified, file plus number, and qualifies nothing. Their §6 is the send gap; k2kremote's is the name-edit cursor. **DELIBERATELY NOT MASS-EDITED.** The 14 are candidates, not confirmed faults: at least one (`docs/E4B_FORMAT.md:857`, "their RESOLUTION_NOTES §2") is qualified by a *pronoun* a same-line regex cannot see, which is s3ked's cross-line-break case in same-line form. **Every claim made about a tree tonight without grepping it was wrong** — mine about their headings, theirs about our citations, mine about k2kremote's sigil — and a batch rewrite of references I have not each read would be that mistake at scale. **To fix:** read all 14, attribute each to the project whose section the cited content actually matches (the one live case, `k2kremote §6`, was verified this way — our text is about the name-edit cursor, which is `RESOLUTION_NOTES.md §6` there and "Object lists — not covered" in their `MAC_FORMAT.md`), and prefix the project. **Also note k2kremote numbers PER FILE**: `RESOLUTION_NOTES.md` 1..73 and `MAC_FORMAT.md` 1..7, sharing every number in that range, so a citation of theirs at 7 or below must name the file as well as the project. `test_citations_resolve` cannot help — it resolves NAMED sections only, so numbered citations are unchecked by construction. |

| ~~**AKAI: LFODEP and LFODEL were never written**~~ **FIXED 2026-09-15** | *(found while implementing s3ked §255's 1:1 rule)* Program offsets **0x22 (LFODEP)** and **0x23 (LFODEL)** were unassigned for the life of the writer — verified by scanning every `p[0x..]` assignment in the source, and by their absence from `_PROGRAM_HW_DEFAULTS`. So every program this project produced carried the LFO→pitch routing gate ON (repaired 2026-08-23) and the **depth at zero**, which with the measured `depth = min(99, LFODEP + MWLDEP·wheel/127)` and our unconditional `MWLDEP = 30` played a **fixed vibrato of 30 at full wheel and none at rest, whatever the source asked for**. Same shape as the routing bug whose fix-comment sits directly above it: *a source's vibrato carried faithfully to the last byte and then dropped* — the routing was repaired, the depth was not. **It survived every round-trip check because the direction that loses data is not the direction anything tests**: 206 of 213 real AKAI programs carry LFODEP 0, so a round trip reproduces zero perfectly. The loss is one-directional, on the **21.9% of MPC instruments (18,102 of 82,581) that state a non-zero `LfoPitch`**. **Fixed** with `akai_pitch_to_lfo_depth` and `akai_delay_seconds_to_byte` (exact inverses of the reader's laws, round-tripping every byte 0..99) plus `akai_lfo_depth_split`, which divides a depth by `wheel_to_lfo` as `LFODEP = D·(1−Kw)`, `MWLDEP = D·Kw` — the split is only expressible because s3ked §255 measured that MWLDEP **adds** rather than scaling. **This deliberately changes the unconditional `MWLDEP = 30`**: a source stating a vibrato with no gating now gets its whole depth in LFODEP and 0 in MWLDEP. The old 30 was right by coincidence of the factory value on a round trip and wrong the moment a source specified anything. 36 tests in `tests/test_akai_lfo_depth.py`; **negative control run** — the writer regression fails when 0x22/0x23 stop being assigned, and it drives `_program_common` directly rather than reimplementing the split, because the failure *was* that nothing assigned the byte. **Open:** confirm by ear that an MPC vibrato now arrives on the S3000XL at the depth the source asked for. |

| ~~**`AKAI_MODVAMP3_KG_OFFSET = 155` is probably not MODVAMP3**~~ **DOUBT WITHDRAWN 2026-09-15 — nothing convicted the offset, and two of the three arguments were mine** | *Kept because the failures are worth more than the conclusion.* **(1)** "Stored values run −116..+127, past the ±50 rail." Dead: s3ked §256 wrote 90 and 166 to each of 151–155 over SysEx and every one read back verbatim, surviving a wait, a program change and sounding the voice — the rail belongs to the write path, not the field. **(2)** "155 doesn't pile on ±50 like its four neighbours (0.73 % against 4.90–16.69 %)." Dead, and this was my second try after the first failed. s3ked's caveat was that a rail-pile rate is a **usage** statistic, not a field property; I agreed in writing and didn't test it. Testing it takes one query and ends the argument: across the eight program-level mod amounts the rate runs **0.00 % to 37.67 %**, and `MODVAMP2` — a loudness amount nobody doubts — has 392 non-zero values and **not one** on the rail. The narrow band was an accident of picking four neighbours. **(3)** s3ked's own counter, "155 piles at 127, so it's a 0..127 limit field like HIVEL": dead too, and the corpus settles this one outright — a 0..127 field never sets the high bit, and across 209,592 HIVEL slots **0.00 %** do against 155's **0.49 %**. It is signed, as `MODVAMP3` should be. Also tested and rejected: one library's authoring tool (excluding all 17 volumes that contribute an out-of-rail value leaves the rate at 0.45 %). **And a correction I owed s3ked**: I told them the offset carried no provenance. True of the constant's comment, false of `docs/AKAI_S3000_FORMAT.md`, which records 151–155 as transcribed from Akai's own parameter document — I had a sourced offset and spent two arguments attacking it. No resave needed; the request to Jan is withdrawn. |

| **`AKAI_MODVAMP_PANEL_RAIL = 50` — confirmed, with the trap that nearly cost me the offset above** | *(2026-09-15)* Corroborated over 64 images / 9,465 S3000 programs by material piling on the bound: program 92/93 run exactly −50..+50 over 18,930 slots, 77 on the bound, none past; keygroup 151–154 put 4.9–16.7 % of non-zero values on ±50. It is a **panel** rail, not a stored-value invariant — the byte-offset SysEx path accepts 90 and 166 verbatim (s3ked §256) — so **our reader must clamp rather than assume the range**, and real material contains the counterexamples (7–57 stragglers per keygroup field). **The trap, now written beside the constant: a pile is evidence the rail is there; a missing pile is evidence of nothing.** The rate is driven by how often anyone pushes that particular parameter to its limit, which across eight fields varies from 37.67 % to 0.00 %. |

| ~~**`KRZ_RES_KEYTRK_DB_PER_KEY` assumed to hold off the lowpass**~~ **CLOSED 2026-09-15 without a measurement — the exposure is 0.2 %** | *(Jan: "the 'assumed' ones, can any of those be done autonomously?")* The law (`resonance(key) = Adjust + KeyTrk × (key − 60)`) was measured on the lowpass only, and its own comment warns against assuming the other filter types follow. The writer assumes it anyway: `_patch_layer`'s resonance branch covers the Alg-1 highpass (54), TWIN PEAKS bandpass (55) and DOUBLE NOTCH (56) as well as the lowpass (50). **Both sides of the conversion were counted, because they are different populations and only one of them is where the constant is consumed.**<br><br>*Write side (where it is used).* A ramp is only written when fusion merges >3 voices whose `filter_resonance` differs AND the surviving `filter_type` maps to a non-lowpass Alg-1 filter: **20 of 10,003 E4B instruments (0.20 %) and 4 of 267 XPM instruments (1.50 %)**. That is a ceiling, not a count — `_reson_keytrack_fit` discards the ramp unless it beats the flat value by 5 %.<br><br>*Read side.* **88 of 43,424 KRZ F2 blocks (0.20 %)** carry a non-zero KeyTrk on a filter we can name that is not a lowpass. Upper bound **845 (1.9 %)** if every one of the 757 blocks whose F2 type code is still unclassified turned out to be a non-lowpass filter.<br><br>**A correction belongs in the record.** Mid-scan I was about to report that "of the 3,118 blocks with an actual filter, 2,384 (76.5 %) are NOT lowpass". That was wrong by the width of the conclusion: **63.8 % of the KeyTrk-bearing blocks carry a type code `_K2_NON_FILTER` lists as not a filter at all.** Of the 822 that sit on a known filter, **734 (89.3 %) ARE the lowpass** — the opposite reading. Status: closed by prevalence. Reopen if someone has a K2000R idle and wants the highpass and bandpass gradients for completeness; nothing in the pipeline is waiting on them. |

| ~~**`KRZ_F4_AMP_ADJUST_DB_PER_UNIT`'s ±96 rail borrowed from a neighbour**~~ **CLOSED 2026-09-15 — as far as a corpus can close a clamp** | *(same sweep)* The dB/unit value is measured; only the ±96 bound was inherited from a sibling parameter. Histogrammed at `seg[1]` of HOB `0x53` over **43,424 blocks, 92.0 % of them non-zero**: **range −96 .. +42. Eight blocks sit exactly on −96 and not one passes it in either direction.** The negative bound is corroborated to the byte by material that reaches it and stops. The positive bound is simply never approached — the largest value in the corpus is +42, fifty-four short of +96 — so a wrong positive rail cannot reach any real material. Exposure if the negative rail is off by a step: **8 blocks, 0.018 %**. Status: closed. A panel read would only be confirming a bound the corpus already respects 43,424 times out of 43,424. |

| **`GIGVELFILT` — the GIG reader states a velocity→filter depth in a unit no machine has** | *(2026-09-15, from eosed §143)* `gig_parser` turns a GIG velocity→cutoff FRACTION into cents through `VEL_FILTER_FULL_CENTS = 9120`. Two things are wrong with that and only one of them is the number.<br><br>**The number describes nothing.** It was never measured — it is `FILTER_ENV_FULL_CENTS` times the Vel+ source's 2.08-unit reach. eosed measured the real thing this morning at three base cutoffs: **a cord adds FMORPH bytes, not cents** (the same shift spreads 9.5 % in cents across bases and 1.6 % in bytes), and **the E4XT's entire filter travel is about 7829 cents**, byte 20→251. So 9120 overshoots the whole destination by ~1291 cents. Every +100 % cord in their first run saturated for exactly this reason — and their saturation guard is the only thing that stopped three confident base-dependence figures being read off where the sound card ran out.<br><br>**The axis is wrong too, and that is the part re-measuring cannot fix.** A GIG fraction means whatever GigaStudio meant by it; an E4XT span cannot supply that, whatever its value. XPM was moved off this constant onto its own measured `MPC_FILTER_MOD_FULL_CENTS = 11409` on 2026-09-01 — the GIG path was not, because there is nothing measured to move it to.<br><br>**Not urgent, and the write side is already correct**: `e4b_writer` routes velocity→filter through `e4xt_cents_to_cord_amount` from the voice's own base (`_cord(..., E4XT_VEL_SOURCE_UNITS)`), so it never reads this constant and saturates honestly at the machine's own limit. Corpus exposure on the read side: 155 GIG files, 160 voices, velocity→filter non-zero on **15 (9.4 %)**, 215..8976 cents, median 1436, one past the E4XT's travel. A small count — but the error is multiplicative, so all 15 are wrong by whatever factor the real GIG span differs by. Blocked on: what a GigaStudio velocity→cutoff fraction actually means. Documentation, not a bench. |

| ~~**§109's write path was never recorded**~~ **ANSWERED 2026-09-15 — there is no opcode** | *(s3ked asked which write path our `MODVFILT1 = 90 → 50` clamp used, correctly noting that the entry stated it as a property of the FIELD.)* **This project has no AKAI parameter-SysEx path at all** — every AKAI measurement here goes through a generated disk image (`tests/re_banks/gen_akai_*_disc.py`), loaded by Jan, measured from sound. The only SysEx in `akai_s3000_writer.py` is comments citing s3ked. So the 90 went into a program inside a disk image, was loaded from disc, and the 50 was read off the panel.<br><br>That makes §109 and s3ked §256 a pair rather than a conflict: **the clamp lives on the disk-load/panel path and is absent from the byte-offset SysEx path**, the same split §13a found for the delete-on-duplicate-name rule. Recorded in the writer beside the measurement. Consequence for us: our output only ever reaches the machine through the clamping path and `akai_velocity_filter` clamps to ±50 before writing, so nothing we generate can exercise the gap — it matters only when READING third-party material. |

| ~~**`models/common.py` and `docs/AKAI_S3000_FORMAT.md` disagreed about whether the mod-matrix offsets had a source**~~ **FIXED 2026-09-15, as a mechanism** | *This was the root cause of the whole offset-155 episode.* The doc recorded 151–155 as transcribed from Akai's own parameter document; the constants carried nothing. An offset that **reads** as unsourced is cheap to doubt, and it cost two wrong hypotheses, a retraction, and a request to Jan for a disk write that was not needed. Adding a `see AKAI_S3000_FORMAT.md` comment was the obvious fix and is **not sufficient**: a pointer is a rule, and the rule that catches a drifting rule is the one nobody remembers to apply. So `tests/test_akai_modmatrix_doc_agrees.py` parses the mod-matrix table out of the markdown and compares it to the imported constants. A renamed or missing row fails loudly rather than passing on an empty list — the vacuous-pass shape that two earlier audits in this repo had. Both negative controls verified: breaking the constant fails it, breaking the doc row fails it. It reads markdown and compares imported VALUES, so it is clear of `test_no_test_here_infers_a_code_property_from_source_TEXT`. |

| ~~**The AKAI reader took MODV amounts unclamped, so it reported depths the sampler never plays**~~ **FIXED 2026-09-15** | *Found by checking a claim I had just written into a comment.* I documented "a reader must clamp rather than assume the range" beside `AKAI_MODVAMP_PANEL_RAIL` — then checked, and **our own reader did not**: `akai_s3000_parser` took both amounts through a bare `_s8()` sign conversion. Not cosmetic, because both reach conversion output — `mod_amount_amp3` sets the tremolo depth and `mod_amount_filt1` the velocity→filter span.<br><br>The ±50 rail belongs to the panel and the disk-load path, not to the stored byte (s3ked §256 vs our §109), so real third-party material carries values the machine will not play. Over 52,398 corpus keygroups: **80 (0.153 %) tremolo amounts and 7 (0.013 %) filter amounts changed**, the worst over-reading its true depth by **2.54×**. Fixed with `_s8_rail()`, on the principle already stated for `e4xt_cord_amount_to_cents` — the model describes the sound, not the request.<br><br>`tests/test_akai_modv_rail_clamp.py`, 14 cases: in-rail values pass through untouched (so a clamp-everything-to-zero bug fails), out-of-rail values clamp, **the clamp is asserted to DIFFER from plain `_s8`** (so aliasing the two fails), and an end-to-end parse asserts the clamp is actually wired in rather than merely defined — that last one verified by reverting the wiring and watching it fail, because this repo has had two audits pass while testing nothing. |

| **Does `MWLDEP` raise the TREMOLO too, or only the vibrato?** | *(2026-09-15, opened by the modwheel read side)* The pitch path now reads LFO1's depth as `LFODEP + MWLDEP`, because that inverts what our own writer splits apart. The LOUDNESS path still uses `LFODEP` alone. If `MWLDEP` really raises the program-wide LFO depth, it should feed the tremolo as well and the two paths are now inconsistent; if it is specific to pitch, they are both right. `AKAI_LFO_LOUDNESS_DB_PER_PRODUCT` was fitted against `LFODEP` with the wheel presumably down, so the measurement cannot distinguish the two. **Deliberately not guessed** — extending the sum to loudness would be an unmeasured change to 89.2 % of programs. Blocked on: one bench run — set `LFODEP 0`, `MWLDEP 60`, an LFO1→loudness routing, and listen for tremolo with the wheel up and down. |

| **Velocity → amp-envelope attack: three of four machines done; XPM open** | *(2026-09-15)* Prioritised by corpus across all four formats before building. **AKAI** 19.4 % of keygroups (measured law, s3ked §47). **KRZ** 9.1 % of programs — `ENVCTL Att VelTrk`, ENC `0x20`[4], span = the displayed multiplier exactly, byte→multiplier from a 256-entry firmware ROM table. **E4B** 7.1 % of voices (measured exponent, eosed §144). **XPM** 1.6 %, still open: `VelocityToVolumeAttack` has no measured meaning in time, and it is the weakest of the four.<br><br>**THREE MACHINES, THREE ANCHORS, NO TWO ALIKE** — K2000 at velocity 1, AKAI at 64, E4XT at 127. Assuming an anchor instead of measuring it would be wrong on two machines of three, and wrong *silently*: the routing still sounds like velocity shaping the attack, with the wrong end of the dynamics carrying it. This is why the model carries a pivot beside the pivot-free span, and why every conversion re-anchors the TIME.<br><br>**One coupling to preserve:** the KRZ writer's `ENVCTL Att VelTrk` is live only because `_patch_layer` sets `0x20`[1] = 0 (User) unconditionally. ENVCTL does not touch a *Natural* envelope's attack, so a future change making the mode conditional would disable this routing without touching its code. The reader guards the same way — it ignores `Att VelTrk` on a Natural layer rather than reporting modulation the machine does not apply. |

| **The model unit for velocity→attack was pivot-relative and had to be reworked before anything was built on it** | *(2026-09-15, same day, caught while scoping the E4B path)* The first version stated the depth as t(vel 127) / t(pivot). That needs the pivot to mean anything, and since the machines disagree on where their no-effect velocity sits (AKAI 64, E4XT `Vel<` 127), **AKAI→E4B would have needed a velocity→attack SHAPE that nobody has measured on any machine**. Replaced by the pivot-free span t(vel 1) / t(vel 127), which needs no shape because every machine has both velocities. Two things confirmed the new unit rather than merely excusing it: it **separates all five measured depths** where the old one collapsed +25 with +50, and the hardware comes out **symmetric** in it — consecutive log₁₀ steps of −0.398, −2.077, −2.070, −0.398, the outer pair equal to three digits and the inner pair to three. A quantity in which the machine's own behaviour is symmetric is usually the machine's own quantity. The pivot is still carried, because it says at which velocity the separately-stored attack TIME applies. |

| ~~**E4XT velocity→attack: the law is DERIVED, not measured**~~ **CONFIRMED 2026-09-15 (eosed §144), ~8 % high, corrected** | The composed law survived falsification and came out 4.5 % high in the exponent. The error decomposed into **both** ingredients being slightly generous — our `ENV_RATE_K` 0.0581 against their measured 0.0576 (0.9 %) and 1.27 bytes/amount against an implied 1.226 (3.5 %) — compounding to ~9 % predicted against ~8 % observed. Replaced by the measured composite rather than re-derived.<br><br>**Two assumptions confirmed rather than assumed:** `t(vel 127)` is constant across every amount, so `Vel<` really does deliver zero at full velocity; and base-independence holds (four bases, mean span 7.23 ± 0.35, trend not resolved against the scatter).<br><br>**It read FALSE twice first, and both artefacts pushed the same way as the hypothesis.** A truncated rise shortens the span at high bases; a seven-sample denominator lengthens it at low ones — either alone reads as base-dependence, together they manufacture a clean monotone trend across the whole sweep. The tell was *impossible*, not statistical: a longer base measured a shorter rise, which no monotone rate law permits. **When an artefact biases toward the hypothesis under test, goodness of the trend is no evidence at all.**<br><br>One thing left open: their two runs disagree by 3.7 % at the same amount (7.23 from the base sweep, 7.50 from the amount sweep). We use the amount-sweep exponent 0.07206 — that run measures the quantity the constant *is*, and its two rungs agree to 0.3 % — but nothing here is better than ~2 % until the disagreement is reconciled. No conversion in this project is sensitive at that level. |

| ~~**The attack TIME crossed machines unconverted**~~ **FIXED 2026-09-15** | *Found while building the E4B path, not by a test.* The velocity→attack SPAN is pivot-free and crosses unchanged — that is what the unit is for. `amp_env.attack` is not: it is the time at the SOURCE machine's no-effect velocity, and the machines disagree (AKAI 64, E4XT `Vel<` 127). Carried across unchanged, an AKAI's velocity-64 attack lands in the E4XT's velocity-127 slot **wrong by the size of its own span — up to two orders of magnitude**.<br><br>Both writers now re-anchor, and neither needs a new assumption: the AKAI move is a table lookup, because s3ked §47 measured the rise at velocity 1, 64 AND 127 for all five depths (the re-anchor reproduces the bench times within 2 %); the E4XT move follows from the same exponential byte law as the span. **A machine with no such law must keep the time and say so, not scale it by a plausible guess.** This also retired the earlier "drop a foreign pivot" guard — dropping a routing that converts exactly would have discarded 19.4 % of the corpus for nothing. |

| ~~**MPC AD / one-shot programs are gated at note-off on the E4B path**~~ **FIXED 2026-09-16** | *Reported by Jan from the hardware: `the one-shot piano` "has no release at all, cuts off way too fast".*<br><br>**Cause.** An MPC program in AD / one-shot mode is not gated at note-off — Jan: *"the AD curve is set up in a way that it always runs the whole sample"*, the length living in the sample rather than the envelope. Such a program still carries a `VolumeRelease`, it is **0**, and the machine **ignores it**. Neither `VolumeADEnvelope` nor `OneShot` was read by `xpm_parser` at all, so that ignored 0 was converted literally into E4XT release rate byte **0** — the instant gate — on samples running 0.92–2.66 s.<br><br>**Not an edge case: 21,011 of 23,968 live corpus keygroups (87.7 %) and 1,117 of 1,613 programs (69.2 %)** carry one of the two flags.<br><br>**The fix is the release LEVEL, not the release time**, and that distinction is the whole of it. A long release is a long FADE: the E4XT release falls ~97.8 dB over its duration, so even the maximum 49.6 s still drops a 2.66 s sample ~5 dB before it ends. Both release segments now target **full level**, so the amp does not move at note-off and the voice ends when the sample does. Guarded on the sample not looping — a looping voice held open would never free.<br><br>**Yesterday's audit had already found it.** Both fields were on the `test_source_fields_unread.py` known-debt list — `OneShot` 5,828 instruments, `VolumeADEnvelope` 1,243 — filed as "blocked on deciding which to implement first". The audit named the defect a day before it was heard; what it could not supply was that AD mode means the release is *ignored* rather than *short*, which took Jan at the keyboard. Both now removed from the debt list, which its own staleness guard enforced. **NOT HARDWARE-CONFIRMED** — needs a listen. <br><br>**CONDITION CORRECTED THE SAME EVENING, BEFORE ANY HARDWARE TEST.** The first version gated on `VolumeADEnvelope` **or** `OneShot`. VinSamLib then checked all five banks Jan played: **AD is True on all five**, so it predicts nothing, while only the two DRUM banks carry `OneShot` — and those are exactly the two with a release of 0. `the pad` has AD True, no `OneShot`, and a 2.849 s release Jan confirmed **by ear** sounds right, so gating on AD would have held the amp open on three banks whose releases are correct today and destroyed them. Now gated on `OneShot` alone; `VolumeADEnvelope` went back onto the known-debt list, which its own guard demanded. This also explains a hypothesis that failed its own control on 2026-09-14 — *"AD means the envelope ignores note-off"* was tested against the pad, found the release honoured, and dropped. **It was dead for AD and alive for the flag beside it**, which nobody was reading either.|

| **`--chromatic-pads`: a melodic one-shot kit is unplayable under the MPC's drum pad map** | *(2026-09-16, Jan: "I would rather expect A1→C2, A2→C#2, A3→D2")* The pad→note map in the file is the MPC's **factory drum layout**: all sixteen values are General MIDI percussion notes arranged as a kit on the 4×4 grid — kick and snare on the bottom rows, toms descending across the third, cymbals on top — and the MPC stamps it on every program whatever the pads hold. Hence a piano-chord bank wearing a drum layout. Furniture, not a statement.<br><br>**Honouring it is faithful and stays the DEFAULT**, and that was not a taste call: the chromatic layout is what we did before `f793f56`, and Jan's own listening test disproved it — nineteen notes carried sound on one side or the other with **exactly one agreement**, note 40, the single coincidence between the two layouts (§XPMPADMAP).<br><br>**But faithful is close to unplayable for melodic content on a keyboard instrument.** Measured on the reported bank: faithful spans MIDI 36..82 with **31 silent keys inside the span**; chromatic spans 36..51 with **none**. `--chromatic-pads` is opt-in for exactly that, and is explicit rather than heuristic — the file calls this piano kit `type="Drum"` too, so there is no reliable way to tell melodic content apart, and guessing would reintroduce the very mismatch the default exists to avoid.<br><br>Tests include the control that the two layouts actually differ, so the flag cannot pass by doing nothing. **NOT HARDWARE-CONFIRMED.** |

| **What does the S3000XL do at note-off on a NON-LOOPING sample?** | *(2026-09-16, opened by the one-shot fix)* The E4B path now holds both release segments at full level so a `OneShot` program plays its sample through. **The AKAI cannot say that exactly**: its envelope has a release RATE and no release LEVEL, so the hold-at-full trick is unavailable. The writer now uses the slowest rate the field has — RELSE1 99 = 1.47 dB/s, fading a 2.66 s sample by 3.9 dB over its whole life instead of cutting it in 3 ms at RELSE1 0.<br><br>**That mitigation is safe either way, which is why it went in before the measurement:** if the S3000XL gates a non-looping voice at note-off it repairs it; if the machine already carries one-shots to their end, the release never runs and nothing changes. Guarded on the sample not looping — at 1.47 dB/s a looping voice would ring for forty seconds after the key is lifted.<br><br>**What is NOT known:** whether that machine gates at all. Nothing in our notes answers it, `akai_s3000_writer` has no one-shot concept, and byte 0x13 is documented as a LOOP type rather than a note-off gate. **Blocked on:** one bench check — play a one-shot keygroup, release the key early, hear whether the sample continues. If it does, this mitigation is unnecessary (and harmless); if it does not, it is the whole fix. |

| **The E4B release knee: retune DECLINED on evidence, and the metric that suggested it was wrong** | *(2026-09-17)* The two-segment release sits ~3 dB low through the middle of the fall, audible on `the pad` as *"MPC release is still a bit longer"*. A sweep said a better knee — 33.5 dB at 0.965 T against the current 29.0 at 0.96 — would cut the worst-case error from 9.6 dB to 6.0. **Not done, and it should not be.**<br><br>**The listening test that decided it also falsified the metric.** Worst-case dB predicted `the sustaining synth` would be *equally* affected — same 3.3 dB gap while audible. Jan re-listened specifically for release: **the sustaining synth is fine.** `the sustain-0 synth` cannot contribute either way (sustain 0.00, so its release starts from silence and never runs).<br><br>**What actually predicts audibility is the DURATION of the error while the note is still loud enough to hear**, not its depth. Above −30 dB, the error exceeds 2 dB for **1.491 s** on the pad and **0.260 s** on the sustaining synth — the same deficit, **5.7× apart in persistence**. A 3 dB deficit held for a second and a half reads as "shorter"; a quarter second of it does not.<br><br>**Consequences.** (1) the pad is the most exposed case the format has — sustain 1.00, so the fall starts at peak with nothing masking it, and the longest release on the disc. It is not representative. (2) Retuning the knee moves EVERY decay and release on the E4B path, including two banks confirmed good by ear, to chase the one case that is worst by construction. (3) **If it is ever revisited, the objective must be duration-weighted, not max-error** — optimising the metric used here would tune for a moment nobody can hear.<br><br>**PROVENANCE OF THE TWO OBSERVATIONS, because they are not equal.** the sustaining synth was re-listened on the CURRENT disc (`MPC5_E4B-rehearsal.iso`, built at `0102872`) with attention on the release, against the original — that is a first-hand, targeted result. **`the pad` has NOT been re-listened**; its *"MPC release is still a bit longer"* came earlier and second-hand, from the `padmap` disc. It still applies to the current disc because the one-shot fix does not touch it — `OneShot` is absent, so its release bytes are unchanged (rate 86 → level 90, rate 15 → 0, verified byte-for-byte) — but it is a carried-over report, not a fresh one.<br><br>**The decision holds either way**, which is why it was not blocked on it: if the pad is still long, it is the most exposed case and we accept the residual; if it now sounds fine, there is nothing to retune at all. What a listen would change is only whether the residual is audible on this disc **at all**. |
...
| ~~**`e4xt_resonance_byte` ignores `poles` — E4B→E4B RESONANCE CLIMBS**~~ **ALREADY FIXED — both claims** | *(closed 2026-09-15 by verification.)* The row made two specific, testable claims and both are now false. *"(0.5, 2) and (0.5, 4) both return 86"* → they return **86 and 49**, so the `poles/2` factor no longer cancels. *"byte 40 → 0.38 → 67 → 0.998 → 112, pinned at max"* → repacking byte 40 as a 4-pole gives **40, 40, 40**, stable.<br><br>The second claim — filter-OFF voices gaining resonance because the writer passed `poles=2` while the reader inferred 4 — is fixed by both sides now deriving poles from the written filter byte with the **identical** expression, `4 if filter_byte in (0x00, 0x02) else 2` (`e4b_writer` and `e4b_parser:435`).<br><br>**A note on how nearly this was closed wrongly:** the first check was that the function now takes a `poles` argument, which it always did — the row was about the factor cancelling, not the signature. Reading the row's own numbers and running them is what settled it. |
...
| ~~**`_or_default` exists precisely for this and the envelope path does not use it**~~ **FIXED 2026-09-15** | *(found 2026-08-24 by code review, fixed 2026-09-15 after an audit re-confirmed it.)* `getattr(env,'release',0.5) or 0.5` turned an authored **0.0** into the factory half-second — RELSE1 54, a tail on a patch that asked to stop dead. **Both halves had to change:** the call sites now use `_or_default`, and `_rate_law_value` distinguishes `seconds is None` (missing → default) from `seconds <= 0` (authored instant → byte 0, this field's 23042 dB/s). Fixing only the call site would have landed the 0.0 on the existing `seconds <= 0` guard and produced the same 54 by another route.<br><br>Tests carry the two controls that matter: a **missing** time still gets its default (so the fix cannot be "always write 0"), and a stage with **nowhere to travel** — sustain at full — still gets its default rather than being swept into the new branch. Both reverts verified to fail.<br><br>**WHICH SOURCES REACH IT — checked after a peer run appeared to confirm the fix and did not.** VinSamLib converted five MPC banks to an AKAI target and saw zero-release keygroups writing RELSE1 0, which looked like confirmation. It is not: an XPM `VolumeRelease` of 0 maps to **0.00079 s**, not 0.0, so MPC material takes the pre-existing SATURATION path — which lands on byte 0 too, by another route and with a warning. The run exercised the old code. **`sfz` is the path that reaches this fix**: `ampeg_release` passes through literally, so `ampeg_release=0` is an exact 0.0 — verified end-to-end, model `0.0` → RELSE1 **0**, against **54** before. Asserted in the tests, including a guard that fires if XPM ever starts producing an exact zero. |
...
| **AKAI capacity counts `SampleData` objects, but a stereo sample costs TWO** | *(2026-08-24, code review)* `bank_splitter`'s `(509, 509, 510)` check ignores the `-L`/`-R` split, and `sample_voice_cost()` already exists in that file for exactly this and is not used. 300 stereo samples pass at 300 and produce 600 files; `build_akai_hd_image` then raises **after the split is committed**, and the loose-file path has no check at all. **[VERIFIED OPEN 2026-09-15]** Re-checked: the capacity test is still `len(self._sample_names) + new_samples > max_samples` (`bank_splitter:661`) and `len(needed_samples) > cap[0]` (`:797`), both counting objects. `sample_voice_cost()` **is** now called — but at `:439`/`:490`, for POLYPHONY, not for the directory budget, so the row stands exactly as written. |
| ~~**Program header names skip `safe_filename`, so two programs can collide to one directory entry**~~ **FIXED 2026-09-15** | *(found 2026-08-24, fixed 2026-09-15.)* `uniq` keys on the AKAI **name field**, where the machine resolves zones; `safe_filename` collapses a different set of characters. `#` and `+` are both outside the AKAI charset, so `F#BASS` and `F+BASS` differ by a byte in the header and both sanitised to `F_BASS.P3` — one file, written twice, second wins, a program gone from the volume with nothing said.<br><br>The residual half of the collision the surrounding comment already describes: that one was fixed by uniquing the NAME, which is necessary and not sufficient, because the two axes collapse different characters. Program filenames now carry their own registry. Verified: `F#Bass` / `F+Bass` / `F Bass` produce `F BASS.P3`, `F_BASS.P3`, `F_BASS2.P3` — three files where two were lost — and the writer says which it renamed. |
...
| **`_E4XT_Q_TABLE`'s shape is wrong in 88..112 -- the machine is FLAT there and then STEPS** | *(2026-08-24, eosed)* Measured peak height against a Q=0 reference: **20.56 / 20.25 / 20.67 / 20.72 / 21.03 / 21.05 / 21.05 dB at bytes 88..110** -- 0.8 dB across 22 bytes -- then a **2.62 dB STEP between 110 and 112**, then flat again. Our table has a gradient across that whole region. **So the '108 vs 104 transcription' question barely matters** (a mis-placed anchor interpolates across a region with no gradient) and **the step at 112 is the real feature**. eosed's metric differs from ours in absolute terms (20.6 dB at byte 96 against our 14.39) so the numbers are not directly comparable -- **the SHAPE is the finding**. **[VERIFIED OPEN 2026-09-15]** Re-checked 2026-09-15: the anchors still run `(88, 13.02), (96, 14.39), (108, 15.54), (112, 17.84)` — a **4.8 dB gradient** across 88..112, where the measurement found 0.49 dB of drift to byte 110 then a 2.62 dB step. The shape disagreement stands. Absolute levels are not comparable (the measurement is against a Q=0 reference), but flat-then-step against gradual is a shape claim, and shape is what the table encodes. |
...
| **`e4b_parser` does not read a preset's MIDI program number back** | the writer stores it (TOC byte 31, verified) but `parse_e4b` builds `Preset(...)` without `program_number`, so every preset reads back as 0 and an E4B→E4B round trip silently loses the assignment. Cosmetic for conversion, wrong for round-tripping. Found 2026-08-03 **[VERIFIED OPEN 2026-09-15]** Re-checked 2026-09-15: `program_number` does not appear in `parsers/e4b_parser.py` at all. |
...
Reported by Jan on an MS-20 bass patch ("too much LFO→Pitch"): it has
...
`FEATUREDEMO_02 P003` (an MS-20 bass patch) plays its
...
Symptom (reported by Jan on the MS-20 bass patch): "metallic, out of tune."
...
Verified end-to-end: #204, the MS-20 bass patch (FilterType=19) → ALG2/51/AMP+20 dB.
...
`K2KFEATDEMO` **#204**, the MS-20 bass patch (src FilterType=19, Cutoff=0.27, Reson=0.65):
...
copy", not ISO 9660** — RE'd from a working factory CD (a Kurzweil factory sample CD) + the Kurzweil
...
another sustaining synth 61/61, Keys Timeless 61/61 keys in tune (±60c) — all PASS, no
...
| **AKAI: every looped sample is written "loop in release", probably wrong** | **REOPENED 2026-08-18 — the falsification is withdrawn, its evidence was not our sample.** This row was marked FALSIFIED ON HARDWARE earlier the same day: `NSWHITE` held five seconds flat at −1.7 dBFS with no collapse at t=2.000s, plus an autocorrelation r=+0.9999 at the loop lag. **Both of those measurements were made on audio that does not contain `NSWHITE`.** Checked against the frames we actually wrote, `test2_nswhite.wav` and the "clean" re-run `test2_clean.wav` are **100.0% of their energy in one 250–500 Hz band** — a pure sine at the played note (48→131 Hz, 60→261.5, 72→523.0), a resident program answering because ours shared its PRGNUM and one neighbour was on PMCHAN 255 (OMNI). A held sine has a flat envelope, so it *reads* as "the loop sustains"; that is why nothing looked wrong. **The original claim is not re-established either — it is simply unmeasured.** Re-run on `CALNOISE` (HD7, id 7, PRGNUM 120–123, no OMNI), and gate the capture on `python3 tests/re_banks/is_it_our_noise.py` **before** analysing it — it PASSes the written frames (white slope +3.1 dB/band, lag-1 −0.002) and FAILs every capture in `~/temp/s3ked-logs/`. See §AKAILOOPRATE (retracted). **[VERIFIED OPEN 2026-09-15]** Re-checked 2026-09-15: `k[o + 0x13] = 0  # loop in release` at `akai_s3000_writer:3209`, still unconditional. |
| ~~**KRZ writer folds velocity→filter into a static cutoff**~~ **ALREADY FIXED — row stale by three weeks** | *(closed 2026-09-15 by verification, not by work.)* Picked as the top autonomous candidate off this file and found already done: `94bdb53`, *"write the K2000's own velocity→filter cord instead of folding it away"*, **2026-08-25**, on this branch. The row dates from 2026-08-17 and was never closed.<br><br>Verified end-to-end rather than by reading the diff: a bank written with velocity→filter of 0 / +1200 / +2400 / −2400 cents reads back **exactly those values**, negative included, with the cutoff constant at 1981.9 Hz in every case — so the depth is a VelTrk cord at `hob_f1[4]`, not a fold into the corner. `test_krz_writer.py` already guards all of it: resting cutoff unmoved, the depth byte, the zero case, the negative sign, the sub-16 Hz floor. |
...
One build run emitted two different program sets:
    ...
    target  built     has program A  has program B  rows usable vs the MPC
...
k2kremote measured the audible consequence independently — `P006` at key 60
...
## KRZ→E4B: a 12-string renders 40 dB below a tonewheel organ — correct or ours?
...
**Concretely: TARGET shipped at byte 95 and modulates at 8.85 Hz where the source
...
**CLOSED 2026-09-07 — confirmed in the shipping path, caveat discharged.**
    ...
    MX10 TARGET  byte  95   table  8.85   measured  8.85 Hz
    MX11 TARGET  byte 106   table 11.44   measured 11.44 Hz
...
banks were added. Same five names, same disc id, different indices:
    ...
    MATRIX9   B000 KR-E4   B001 S3-E4   B002 S1-E4   B003 MPC-E4B  B004 TARGET
    MATRIX10  B000 TARGET  B001 KR-E4   B002 MPC-E4B B003 S1-E4    B004 S3-E4
...
## TARGET carries NO pan modulation on ANY of the three cards — all three volumes are pre-pan-writer builds

**Status: CAUSE ESTABLISHED 2026-09-07, needs a rebuild + card write (Jan's).**
eosed measured TARGET->E4XT as a total pan loss: R-L +0.42 dB (the interface
...
**How it happened, and it is a process fault worth naming.** `MX9 TARGET_01` was
built at 20:22 for CD3-MATRIX8b, *before* the AmpPan cords existed in the writer.
When CD3-MATRIX9 was assembled the MPC row was rebuilt and TARGET was **carried
...
actually on the cards tonight:
    ...
    E4XT   MX9 TARGET_01   no AmpPan cord            (rebuild has one, amount 24)
    K2000  MX9TARG.KRZ     0 programs with a PANNER
    AKAI   MX9 TARGET.P3   MODVPAN1 = 0, PANRAT = 1  (i.e. the hardware default)
...
expect TARGET stationary "because no writer emits the depth". That was true when
...
**Fix:** rebuild TARGET for all three targets and write all three cards. Needs
...
**Blocked on:** a ladder. One patch per stage is not a law.
  ...
  decay   (the sustain-0 synth, sustain 0)   asks 3.351 s   measures 2.94 s   13% short
  release (the sustaining synth)         asks 0.497 s   measures ~1.4 s   **2.8x wrong**
