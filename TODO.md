...
| **AKAI->E4B releases are ~1.8x too fast: we match SECONDS across two spans that disagree about where silence is** | *(2026-08-24, §AKAIRELSPAN)* **Measured on both sides.** Source is 15.22 dB/s (s3ked) and our own AKAI law agrees at 15.33; the E4XT delivers 27.2-28.6 (eosed, clean load). The reader converts RELSE1 over a 60.07 dB scale, the writer converts back over 97.82 dB. **The decay agrees to 1-7% and only the release is out** -- decay runs peak->sustain, where both machines' level laws agree, while release runs sustain->*silence*, and neither 60.07 nor 97.82 was ever measured. Both are parameter-scale artifacts. **Fix:** carry the RATE, not the seconds -- optional `release_rate_db_per_s` on `Envelope`, `None` for sources that really do specify a duration (MPC). Do NOT just unify the two span constants: that makes the round trip self-consistent while both sides still assert an invented floor. **Wire together with the rate-anchor item below** -- the two errors are in opposite directions and partly cancel. Also noted, not fixed: the decay spans diverge 1.38x at SUSTN1 90 and 1.83x at 95. |
| **Our EOS envelope rate law is 18-25% fast** | *(2026-08-24, §E4BRATEANCHOR)* We carry `dB/s = 27.9*2**(-(byte-72)/12.3)`. eosed's four fresh hardware points: byte 69 -> 27.36/27.97 measured against our 33.04; byte 80 -> 13.44/14.24 against our 17.78. The **slope** is right (12.3 vs ln2/0.0565 = 12.27) -- it is the anchor, ~3 bytes' worth. Inverting for 15.224 dB/s: eosed's law 79.7, their direct solve 78.1/78.9, ours 82.8. **The smell was already in the file and written off** as *"5%, unexplained and not worth chasing"*. Invisible to our 499 tests because every path uses this law in BOTH directions, so a round trip through our own code cancels it exactly. Replacement law's provenance now supplied: window **60-87**, so 78-83 **interpolates** -- but on **three distinct bytes**, the slope carried by §43's independent filter-envelope figure (12.2 vs 12.27 bytes/halving) rather than by the point count. **Adopt the slope; take the byte from direct measurement, not by inverting the prefactor** -- the two sets use different estimators and the law runs 5-10% fast at byte 80 where it is within 1% at 69. **Blocked on:** eosed's 60-100 OLS sweep (~1h bench, offered) before a constant touching every conversion is treated as settled. |
