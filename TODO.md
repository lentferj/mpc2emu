...
| **AKAI: a LOOPED program plays the sample loaded immediately BEFORE the one it names** | **RULE CONFIRMED 2026-08-18 at nine observations, two of them predicted in advance (s3ked §136); CAUSE STILL OPEN.** One-shots are correct; the three silences are the off-by-one reaching past the start of the pool. `SBADD` resolves by name correctly and order-independently, the machine's own saved copies contain correct audio under correct names, and our disc is verified end to end — entry → block → header → name → PCM → program → keygroup → all four zones. **Three hypotheses died on measurement:** the duplicate stored name (§AKAINAME12), the zone pointer at `+0x16` (0xFFFF is the most common factory value, 35.3%), and the zero sample-data address (§AKAISDATA — predicts white where pink was heard). **Standing limit: not reproduced on factory material**, so this is a claim about discs from our writer until it is. **Next:** `~/temp/HD7_r3.img` carries volumes `SDADDR`/`SDZERO`, identical but for the address field, to give that deviation a consequence. §AKAILOOPCROSS |
