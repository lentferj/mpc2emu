...
| **Filter-envelope depth: the AKAI half is measured, the E4B half is not** | *(open 2026-08-22, §ENV2RESULT / §ENV2CONTOUR)* `akai_filter_env_bytes()` maps the E4B envelope amount to the AKAI depth byte as a straight `amount * 50`. **The AKAI half is now measured** (s3ked §148): `octaves = 0.00283 * SUSTN2 * depth`, product form self-consistent to ±1%, slots 1 and 3 interchangeable, saturating above depth ~20 at base FILFRQ 40 so a converter should clamp rather than extrapolate. **The E4B half is still unmeasured** — how many octaves a FilterEnv→FilFreq cord at amount 1.0 actually moves the corner — so the two cannot be mapped onto each other rather than assumed equal. Also structural: on the AKAI the shift is a PRODUCT of level and depth, and our conversion writes `SUSTN2` and `depth` from independent source fields. **Priority is LOW**: §ENV2CONTOUR showed programs 0 and 1 substantially fixed by the envelope alone (contour error −65%/−59%, r 0.78→0.97), so this is a refinement, not the missing piece. Blocked on: E4XT parameter writes, which Jan has authorised. |
...
| **KRZ filter-env and LFO depths are unmeasured linear scalings** | *(2026-08-22)* `hob_f1[6] = amt * 127` and `cal[22] = lfo1_to_pitch * 79`, both self-flagged `approx` in the code. Same shape as the AKAI's `amount * 50`, which measurement showed delivers ~3 octaves where the source may mean five (§ENV2RESULT). Blocked on the same K2000R session as §KRZCUTCAL. |

| **AKAI filter-envelope attack law is measured and disagrees with the shipped one** | *(2026-08-22, §ATKMEAS)* Measured on hardware across `ATTAK2` 0→72, ten programs, three takes: `t = 0.00250 * exp(0.06723 * byte)`, **doubling every 10.3 bytes against the shipped 7.1**. At byte 72 we predict 1.47 s where the machine takes **0.34 s — 4.3x out**, and because the law is *inverted* to choose a byte, long filter attacks come out far shorter than the source asked. Also settles §ENV2FLOOR: **the field IS live below byte 40** — byte 0 reaches t90 0.04 s sooner than byte 40 — so our clamp costs about 40 ms of attack range. Blocked on: an independent measurement before replacing the constant. The measurement's own limits are a 20 ms resolution and an offset removed by assuming byte 0 is instant; the slope is robust to both, the prefactor is not. |
