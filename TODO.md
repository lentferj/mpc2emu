...
| **Filter-2 bench queue, in priority order** | *(2026-09-10, §AKAIFIL2ONELAW)* **Status: OPEN, all three need a crossing; fold into one scheduled for other reasons.** Ordered so that partial completion still buys the most: **1. High-`FLT2Q` points below rung 45** — the only one that can still overturn something, since it settles whether mode 0's bottom-end flattening is real or the same feature-offset effect that produced four nights of spurious per-mode laws. Nothing on the resident volumes reaches there. **2. A `FLT2Q` sweep of the HP passband bump at one rung** — resonance or structural; two captures. **3. Rungs 88/94 for BP and EQ** — needs `FLTCOMB`, expected to change nothing, but mode 0's own top steepened between 88 and 94 so "expected" is carrying weight. |
| **Is the highpass passband bump a resonance?** | *(filed 2026-09-09, §AKAIFIL2HPREF, s3ked's suggestion)* **Status: OPEN, cheap.** Blocked on: two captures at one rung, foldable into any crossing scheduled for other reasons. The highpass has a **+1.1 dB passband bump at ~3.8x its corner that scales with the corner**, measured at **`FLT2Q` 0 only**. **If it is a resonance it should grow with `FLT2Q`; if it does not, it is structural in the highpass path.** Two captures at one rung separate those. It bears on a bigger question: whether that bump and mode 0's unexplained bottom-end flattening are **the same circuit seen twice** — mode 0 is the denominator under every mode ratio, so knowing what bends it is worth more than the local answer. |
