...
| **Does the XPM reader read the AMPLITUDE envelope at all?** | *(2026-08-23)* Audit of this week's findings against the XPM input path turned this up and could not settle it. **`amp_env` appears ZERO times in `parsers/xpm_parser.py`** — it reads `FilterEnvAmt` and a filter ADSR, but nothing named for volume — yet a parsed voice comes back with `amp_env` populated, so it is either set indirectly or defaulted. **If defaulted, every XPM→anything conversion silently loses the source's amplitude shaping**, and all three writers consume `amp_env`, so it would affect every output format. Could not be resolved tonight: the only local `.xpm` is a minimal generated test file with no envelope tags, so there is nothing to check the reader against. **Needs one MPC-authored keygroup program with a non-default volume envelope.** No hardware required to answer — just the file. |
