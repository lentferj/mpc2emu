...
| **MPC 3 project with several keygroup programs converts only the first** | `parse_xpm` builds one Preset per file, so a `SerialisableProjectData` holding more than one `type == 1` program drops the rest. It warns and names them, so nothing is silent — but the fix is a multi-preset `parse_xpm`, which is a real refactor of a ~390-line function. New 2026-08-03 with §MPC3D3 |
