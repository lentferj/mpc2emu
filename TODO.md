...
| **KRZ filter cutoff is written on the wrong scale** | *(found 2026-08-22, §KRZCUTCAL)* `krz_writer._cutoff_byte` maps the model's 0..1 position LINEARLY onto the K2000's signed-semitone byte, but the model's position spans 57 Hz–20 kHz (8.45 oct) and the byte range spans 16 Hz–25088 Hz (10.61 oct). Both scales are log, so the shape is right and the **endpoints are not** — every interior value is misplaced, worst **1.80 octaves** at the bottom and still **1.17** at position 0.3, i.e. worst exactly where filtered material lives. The AKAI equivalent measures within 0.05 oct. `docs/KRZ_FORMAT.md` records it as a round-trip inconsistency, which understates it: every KRZ we write puts the filter in the wrong place, and the reader uses the other curve so nothing downstream notices. **Material built and on the Gotek**: `CUTCAL_01.img`, 11 presets over stationary noise, root-matched. Blocked on: measuring the corner per preset on the K2000R, then routing `_cutoff_byte` through Hz against measured endpoints. |
