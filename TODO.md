...
| **The E4B release knee: retune DECLINED on evidence, and the metric that suggested it was wrong** | *(2026-09-17)* The two-segment release sits ~3 dB low through the middle of the fall, audible on `Lunar Daze` as *"MPC release is still a bit longer"*. A sweep said a better knee — 33.5 dB at 0.965 T against the current 29.0 at 0.96 — would cut the worst-case error from 9.6 dB to 6.0. **Not done, and it should not be.**<br><br>**The listening test that decided it also falsified the metric.** Worst-case dB predicted `SY Precious` would be *equally* affected — same 3.3 dB gap while audible. Jan re-listened specifically for release: **Precious is fine.** `Sangre` cannot contribute either way (sustain 0.00, so its release starts from silence and never runs).<br><br>**What actually predicts audibility is the DURATION of the error while the note is still loud enough to hear**, not its depth. Above −30 dB, the error exceeds 2 dB for **1.491 s** on Lunar Daze and **0.260 s** on Precious — the same deficit, **5.7× apart in persistence**. A 3 dB deficit held for a second and a half reads as "shorter"; a quarter second of it does not.<br><br>**Consequences.** (1) Lunar Daze is the most exposed case the format has — sustain 1.00, so the fall starts at peak with nothing masking it, and the longest release on the disc. It is not representative. (2) Retuning the knee moves EVERY decay and release on the E4B path, including two banks confirmed good by ear, to chase the one case that is worst by construction. (3) **If it is ever revisited, the objective must be duration-weighted, not max-error** — optimising the metric used here would tune for a moment nobody can hear.<br><br>**PROVENANCE OF THE TWO OBSERVATIONS, because they are not equal.** `Precious` was re-listened on the CURRENT disc (`MPC5_E4B-rehearsal.iso`, built at `0102872`) with attention on the release, against the original — that is a first-hand, targeted result. **`Lunar Daze` has NOT been re-listened**; its *"MPC release is still a bit longer"* came earlier and second-hand, from the `padmap` disc. It still applies to the current disc because the one-shot fix does not touch it — `OneShot` is absent, so its release bytes are unchanged (rate 86 → level 90, rate 15 → 0, verified byte-for-byte) — but it is a carried-over report, not a fresh one.<br><br>**The decision holds either way**, which is why it was not blocked on it: if Lunar Daze is still long, it is the most exposed case and we accept the residual; if it now sounds fine, there is nothing to retune at all. What a listen would change is only whether the residual is audible on this disc **at all**. |
