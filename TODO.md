...
| **KRZ LFO→pitch depth is an unmeasured linear scaling** | *(2026-08-22)* `cal[22] = lfo1_to_pitch * 79`, self-flagged `approx`. k2kremote is measuring it on the same program 250 with the same every-consecutive-byte discipline. Blocks the `MXE4toKRZ` matrix bank, where all 10 voices use it. |
