...
| ~~XPM `direction` (reverse playback) is dropped~~ | **fixed 2026-08-03** (branch `cwm_ketchup`) — reversal baked into the PCM after slicing, loop points mirrored. Byte-exact against a real reversed sample and mirror-exact in the audio domain. **Needs HW-1** (does `direction` mean whole-region reversal, and does the amp envelope stay forward?) — plan in `docs/re_procedures/mpc3_xpm_params.md`. §XPMGAPS |
| ~~XPM loop crossfade is dropped~~ | **implemented 2026-08-03** (branch `cwm_ketchup`) — equal-power blend baked in, unit-verified byte-exact. **Caveat:** every layer in the corpus has crossfade 0/-1, so it has never run on real data and the MPC's own frame alignment is still unverified. **Needs HW-2**. §XPMGAPS |
