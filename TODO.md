...
Reported by Jan on `patch I Acoustik.xpm` ("too much LFO→Pitch"): it has
...
`FEATUREDEMO_02 P003 patch I-Patch` (= `patch I-Patch 2c.xpm`) plays its
...
Symptom (reported by Jan on `patch I-Patch 2c.xpm`): "metallic, out of tune."
...
Verified end-to-end: #204 patch I-Patch (FilterType=19) → ALG2/51/AMP+20 dB.
...
`K2KFEATDEMO` **#204 patch I-Patch 2c** (src FilterType=19, Cutoff=0.27, Reson=0.65):
...
One build run emitted two different program sets:
    ...
    target  built     patch J        patch L        rows usable vs the MPC
...
k2kremote measured the audible consequence independently — `patch G` at key 60
