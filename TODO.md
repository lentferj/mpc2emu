...
| **Filter-2 bench queue, in priority order** | *(2026-09-10, §AKAIFIL2ONELAW)* **Status: OPEN, and NONE of it needs a card swap** (established 2026-09-10). All SysEx against volumes already resident. Ordered so that partial completion still buys the most: **1. High-`FLT2Q` captures at rungs 30 and 37, ALL THREE MODES** — the only one that can still overturn something, since it settles whether mode 0's bottom-end flattening is real or the same feature-offset effect that produced four nights of spurious per-mode laws. **All three modes, not one**: high-Q convergence onto f0 is verified at rungs 45/64/80 only, so a single mode departing from the law at rung 30 could equally be the bend being real or the modes simply not converging down there — assuming the convergence would be assuming what the experiment tests, which is exactly the fault that produced the spurious laws. Nine captures. Reachable with the resident noise volume: at `FLT2Q` 31 the feature is f0, **56 Hz at rung 30 and 92 Hz at rung 37**, and the earlier failure at these rungs was a shallow −3 dB corner near 21 Hz — a different measurement entirely, so it says nothing about this one. **2. A `FLT2Q` sweep of the HP passband bump at one rung** — resonance or structural; two captures. **2. Rungs 88/94 for BP and EQ** — needs `FLATCOMB`, which is **`FLATCOMB.S3` in the `FILTER2` volume on HD4, resident on the card** (verified 2026-09-10). No crossing; but `loadvolp.py` does CLR-then-load, so it evicts `TC10 NOISE` and getting back is a second load — ~86 s each, so sequence it around whatever else needs the noise. Expected to change nothing, but mode 0's own top steepened between 88 and 94 so "expected" is carrying weight. |
