...
| **Every MPC tremolo is silently dropped on the E4B path** | *(2026-09-14, found auditing constant provenance)* The E4B writer emits **no LFO→AmpVol cord at all** — `_extra_cords` carries LFO1/2 to Pitch, Filter-Freq, Filter-Q and AmpPan and stops there. `E4B_LFO_VOLUME_FULL_DB = 24.0` exists, is marked **UNMEASURED** in its own comment, and is **referenced by nothing in any conversion path** (whole-repo grep: the definition, one neighbouring docstring, a RESOLUTION_NOTES line, and an entry in `test_docs_constants_agree`). So the constant is not miscalibrated — it is unwired, and the feature behind it was never written. **Corpus: `LfoVolume` is present on 82,581 expansion instruments and non-zero on 13,987 — 16.9%.** All of it is lost: not converted wrongly, not approximated, absent. This is the one class of defect our diagnostics cannot warn about, because nothing in the path ever reads the field. **Blocked on** the polarity/reference measurement (eosed, in progress 2026-09-14): `Lfo1~` (id 96, bipolar) and `Lfo1+` (id 97, unipolar) differ by a factor of two at a given amount, and since we write neither today the choice is still open. `lfo_volume_depth_to_amount`'s docstring carries the related open question — whether the swing's peak rises above the unmodulated level (costing depth/2 of headroom, able to clip a zone near full scale) or merely ducks below it; that assertion "rested on nothing" and was withdrawn 2026-09-01. Fix strategy in `docs/RESOLUTION_NOTES.md` §E4BTREMOLO. **[MY 2026-09-15 CLOSURE WAS WRONG — REOPENED 2026-09-17]** I closed this as *"not worth building, `LfoAmount` and `LfoFilter` zero across all 1,635 instruments"*. **Those tags do not exist.** The real ones are `LfoVolume`, `LfoPan`, `LfoCutoff` and `LfoPitch`, and I measured two names that appear nowhere in an XPM — a vacuous measurement used to close a row, which is the exact failure this file catalogues elsewhere.<br><br>**The real prevalence, over 1,624 instruments / 23,995 live keygroups:**<br>`LfoVolume` 2,336 kg (9.7 %), 117 programs (7.2 %), **71 distinct values**<br>`LfoPan` 2,731 kg (11.4 %), 159 programs (9.8 %), 91 distinct<br>`LfoCutoff` 3,012 kg (12.6 %), 189 programs (11.6 %), 104 distinct<br>`LfoPitch` 3,989 kg (16.6 %), 320 programs (19.7 %), 29 distinct<br><br>**Found by Jan at the MPC, not by any test here**: `Pad-PRO5 Lunar Daze` keygroup 4 shows LFO Tri 0.20 Hz with DESTINATIONS PITCH 0, FILTER 0, **AMP 29, PAN 81**. Verified end-to-end: `LfoVolume` is read **zero times** by `xpm_parser`, and the E4B writer's `_extra_cords` carries LFO→Pitch/FilterFreq/FilterQ/**AmpPan** and no LFO→**AmpVol** (0x40). **So AMP is lost twice over; PAN survives** — confirmed by writing a bank and reading the cord table back (slot 8, AmpPan, amount 103).<br><br>**The E4XT half needs no new measurement:** a cord into AmpVol is **0.96442 dB per amount unit**, r² 0.999917 over eight rungs spanning both signs, taken with a constant DC source so there is no sine to fit or period to smear. **The MPC half does:** nothing here says what `AMP 29` means in dB on that machine, and inventing a nominal for it is what produced the `VEL_FILTER_FULL_CENTS` mess. **Blocked on:** one MPC measurement — play a pad at AMP 0 and at AMP 29 and report the peak-to-peak level swing. |
