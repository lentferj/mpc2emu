...
| **KRZ reader under-reads envelope attack time — measured, both target formats** | *(2026-08-22, §MATRIXRESULT)* The conversion matrix measured all six loadable routes on hardware. **Both KRZ-sourced rows attack far too fast: −0.77 octaves (KRZ→E4B) and −0.89 (KRZ→AKAI)**, i.e. conversions reach full level ~1.7–1.9× sooner than the K2000 original, and come out 0.9–1.3 octaves darker late in the note. The same offset through **two independent writers** points at the reader. **And it separates**: with the K2000 as the *target* instead (E4B→KRZ) attack matches at +0.06, so this is not the machine rendering attacks slowly — it is `parsers/krz_parser` decoding envelope attack times too short. Chase in code, no hardware needed. The late-brightness half may share a cause with the ENV2 depth clamp (the source bank stores byte 127 on all 44 routings, which the reader clamps to amount 1.0). |
| **KRZ writer: E4B→KRZ conversions land ~10 cents sharp** | *(2026-08-22, §MATRIXRESULT)* Row median +9.5 ct against the source, consistent across all 10 patches and confined to that one row — so it is the KRZ writer or the K2000's own tuning, not the source material (which is itself ~60 ct sharp and reproduced by the AKAI writer to within **0.2 ct**). Small but systematic; the AKAI row shows −0.2 ct on the same sources, which is the control. |
