...
| **The AKAI mute cut needs a HOLD stage and `Envelope` has no way to say it** | *(2026-08-25, §155 amendment)* s3ked measured the choke at 2 ms resolution: the cut layer stays at **FULL LEVEL for 4-5 ms**, then ramps to -20 dB by 12 ms. Our `Envelope` is four parameters, so with attack 0 and sustain 0 the level falls from t=0 at a constant rate and the hold is unrepresentable. `AKAI_MUTE_CUT_SECONDS` is now set from the measured RAMP (0.058 s = 97 dB at the rate reaching -20 dB at 12 ms), which leaves a **known residual of about 3.6 dB** over the first 30 ms. **Do not close it by refitting the constant** -- matching the energy instead needs 80 ms and puts the ramp 4.5 ms late, trading a shape error for a timing error. The fix is a model that can express a hold: the E4XT's own envelope has two attack and two decay stages and could render it exactly, and the AKAI writer would round-trip it. **Blocked on** deciding whether `Envelope` grows a hold field or whether the mute cut becomes a separate carried quantity like `release_rate_db_per_s`. |
| **AKAI filter key-follow converts far too large, and the first hardware test of it FAILS** | *(2026-08-25, bench, §AKAIKEYFOLLOWHW)* `filter_keytrack` landed 2026-08-24 (`9ff8a7d`) and had never been heard. Measured tonight on a controlled pair — the SAME preset sent twice, once without the Key->FilterFreq cord and once with it, nothing else different on the bench: **turning key tracking on makes the conversion much worse.** Level error against the AKAI source at note 96 went **+8.7 dB -> -8.4 dB**, at note 80 **+7.4 -> +12.5**, and the third-octave distance at those two notes went **5.27 -> 12.87** and **4.54 -> 16.44 dB**. The keyboard slope flipped sign, +7.7 -> -9.4 dB: the correction overshoots by roughly twice. **The chain under test:** AKAI K_FREQ -4 -> `K_FREQ/12` = -0.333 oct/oct -> `/ KEY_FILTER_OCT_PER_OCT` (0.713) -> cord -46%. One of those three steps is wrong; the measurement cannot yet say which. **The response is NOT monotonic**, which is the useful clue — note 96 fell 17 dB while note 80 ROSE 5 dB, so the corner is moving through a resonant peak (we write Q 0.59-0.70 on these voices) rather than simply darkening. A pure gain or scale error could not do that. **Blocked on:** deciding whether K_FREQ/12 is the right law (s3ked measured the corner shift in FILFRQ units per K_FREQ step, not per KEY), and whether the E4XT's Key cord shares the AKAI's pivot note 64. **Do not ship key tracking on the AKAI path until this is settled** — the presets on the bench have it absent and measure better for it. |
