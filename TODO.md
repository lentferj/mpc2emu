...
| **KRZ conversions not rebuilt with the measured pan calibration** | *(2026-09-18, Jan: "postpone krz, record it as open")* The K2000 LFO->pan depth was measured and wired the same day (`KRZ_LFO_PAN_DEPTH_SCALE = 0.4377`, dc7ba89): **0.372 dB/byte, constant to +/-0.003 over a 10x range of the parameter** — the most linear of the three targets — putting the MPC's 5.11 dB reference at byte 14 where the writer had been emitting byte 32.<br><br>**Status: the CODE is done; only the rebuilt conversions are outstanding.** Jan asked for E4B and AKAI to be rebuilt first (VinSamLib, one bank and one volume respectively) and for the KRZ rebuild to wait. Nothing blocks it technically.<br><br>**Blocked on:** a listening pass. The E4XT pan value is the only one of the three confirmed by ear; the AKAI and KRZ figures are measured and unheard. Rebuilding KRZ before the first two are judged would put three unheard changes in front of a listener at once, which is how a wrong one gets attributed to the wrong machine.<br><br>**When it is picked up:** the sweep and its caveats are at `KRZ_LFO_PAN_DEPTH_SCALE`, and the thing to re-read first is §K2PANWIRES — a K2000 PANNER doubles its wire and does NOT itself pan, so a program whose OUTPUT wires are left centred sums them and is silent however hard the panner is driven. Our writer spreads them (`0x70` / `0x90`, opposite signs: `0x90`/`0x94` are BOTH pan -7 and sum exactly like centred wires), so converted programs are unaffected — but any hand-built comparison program needs the same two bytes or it will measure zero and look like a broken conversion. |
