...
| **AKAI S3000 support is cross-verified but not hardware-verified** | Reader and writer agree with **akaiutil**, an independent implementation of the same undocumented format — it accepts our files, reads the metadata back correctly, and its WAV exporter returns our PCM byte-identical. That means we match the best available *interpretation*, not that Akai's ROM agrees. **Blocked on:** real S3000 media and the S3000XL itself. **The `akai-s3000xl` branch is not to be pushed until hardware confirms it** (Jan, 2026-08-05). First checks: load a written program and read its zone names; convert a program the S3000XL saved itself and diff it against one we wrote — that single comparison settles the unused-zone encoding and the `??` regions together. This now covers the **disk images** too: mounting one is the first thing to try, because a disk the sampler will not mount makes every file-level question moot. §AKAIIMG |
...
| **AKAI stereo samples are mixed down to mono** | *(unblocked 2026-08-17 — the hardware question is answered)* **The sampler does NOT pair `-L`/`-R` by itself.** Measured on the S3000XL: a program referencing only `STPAIR-L`, with `STPAIR-R` present on the disc but named by nothing, loaded after CLR with `samples ['STPAIR-L']` — the right half did not arrive. Same for the 12-character case `STEREOLONG-L`, which resolved correctly, so the name-budget edge is fine in itself. Directory and `RSLIST` name forms agreed, so the absence is real and not the §AKAINAME artefact. **So stereo support is possible but must emit BOTH references explicitly**, and the failure mode is silent: a program naming only the left half loads cleanly, raises no error, is *not* dangling (nothing it asked for is missing) and plays mono — invisible to the machine, to `collect()` and to `silence_audit.py`. Any implementation needs its own both-halves-referenced check. Mixdown is unaffected and remains honest. §AKAISTEREO |
...
| **AKAI: every looped sample is written "loop in release", probably wrong** | `akai_s3000_writer` line 642 writes `h[0x13] = 0 if looped else 2`. Per §AKAI_S3000_FORMAT the playback-type values are **0 = loop in release, 1 = loop until release, 2 = no looping, 3 = play to sample end**. For a SUSTAINED looped sample the wanted mode is **1** — loop while the note is held. Mode 0 loops only during the release, so a held note plays the sample through once and then stops, which is the sound / decay / gap / repeat structure s3ked recorded from our noise source, with the period tracking playback rate. **Affects every looped sample we write, not just bench discs.** The existing comment justifies 0 only against our own parser ('the parser treats only 2 as unlooped'), which is a round-trip argument and says nothing about the hardware. **Not yet confirmed:** it predicts a held note goes silent after one pass, testable in seconds with `NSRULER` (`/home/lentferj/temp/NSRULER.S3`, a 1 kHz burst every 100 ms) — the click count before the gap gives the answer directly. Found 2026-08-17 while chasing a 610 ms silent lead-in that is NOT in the file. §NOISESRC |
...
| **KRZ: three velocity→filter topologies, and we read one** | Measured by k2kremote across 30 ROM filter pages, 2026-08-17. **(1) velocity as SOURCE** — `Src2 = AttVel`, which is our 47. **(2) velocity as DEPTH CONTROL** — `Src2 = FUN4, DptCtl = AttVel, MaxDpt −1500/−1000/−1600 ct`: a FUN modulates the corner and velocity scales how much, over a NEGATIVE range, so harder playing sweeps the corner *down*. A scan keyed on `Src2` can never see these. **(3) the direct `VelTrk` field**, which we do not read at all. MaxDpt signs across the 30: 6 negative, 16 zero, 8 positive — our 47 were all positive, so the negative shape is unhandled too. **And 43% of filters are not in slot F1**: 17 at F1, **13 at F3**, with algorithms 4 and 10 appearing in BOTH lists, so slot is not a function of algorithm and cannot be inferred. We read tags `0x50`/`0x51` only. **k2kremote withdrew their frequency fit** — it got worse with more data (VelTrk 62.5%→46.7%, Pad 12.5%→30.0%), so the tidy correspondence was an artefact of eight samples and is not a prior. **Blocked on:** loading `ACOUSTIC.KRZ`, already on the K2000's own disk at `\-SNDSETS\SOUNDART\MS_ACO~1\ACOUSTIC.KRZ` — a two-minute job for Jan, no card shuffle. Join on KeyTrk (14 distinct) or VelTrk (14 distinct); not Fine, which is zero everywhere. Data: `/home/lentferj/temp/rom_fields32.jsonl`. Layer 1 only. §AKAIVELFILT |
| **KRZ→AKAI velocity→filter: the model collapses a RANGE into a scalar** | **Found 2026-08-17 by Jan playing a converted cymbal program: a zone that fires and makes no sound.** The K2000 carries a *pair* — `MinDpt 0 ct`, `MaxDpt +10800 ct` — so velocity only ever OPENS the filter and the base cutoff (196 Hz) is a floor the source never goes below. `MODVFILT1` is **bipolar about a velocity pivot**, and we write `FILFRQ` = the source's *velocity-zero* value, so below the pivot our cutoff falls below a point the source never reaches. Zone 1 (vel 0–63) lives entirely there. **`velocity_to_filter` is one scalar, so `(0,+10800)` and `(−5400,+5400)` normalise identically — different patches the model cannot distinguish.** Directly the consequence of §K2DSP's recorded decision to leave `MinDpt` unwired because the pair 'has nowhere to go in a model that carries one scalar depth': true, and that missing pair is what silences a zone. **The ×25 is NOT the fault and is measured** — the writer records an S3000XL sweep at `FILFRQ 48` where byte151 25 and 50 differ by 0.1 dB, because the field saturates once the quiet end bottoms out. **The same table holds the real fault:** at byte151 **0**, with no modulation at all, v30 already reads **−68.7 dBFS** — `FILFRQ 48` silences it, not the modulation. That was measured 2026-08-16 and read as a property of the field rather than a consequence of the base.WORSE** (more depth closes further below the pivot). `MODVFILT1`'s depth has never been measured — absent from s3ked's `scales.py`; §109 gives only the ±50 clamp. `FILFRQ` *is* measured (`Hz = 6.4597·exp(0.071·FILFRQ)`, r² 0.99984 → 9.76 units/octave), so if a `MODVFILT1` unit is a `FILFRQ` unit our 25 buys 2.56 octaves against the source's 9 — and **±9 octaves is not expressible at all**, the whole `FILFRQ` range being 10.14 octaves. So the fix is a model that carries the pair plus a **documented lossy clamp that reports what it could not represent**, not a new constant. **Blocked on:** measuring `MODVFILT1` octaves-per-unit and its pivot — mid-range `FILFRQ`, resonance-peak tracking not centroid (§108), away from both ends (an `ATTAK2` threefold 'depth-dependence' was the corner saturating). **No static check can see this:** `silence_audit.py` returns clean — no dangling ref, no empty PCM, no velocity gap, every sample resident. The fault exists only in the COMPOSITION of two individually correct translations. §AKAIVELFILT |
| **AKAI: modulation-source codes are validated; 8–12 untested** | The S3000XL zeroes illegal source codes in the 13-byte matrix at program `0x4c`–`0x58` **on load**. Measured 2026-08-17 across three probes: **kept 0–7, 13, 14; cleared 15–23, 24–27, 76–88.** The break is between 14 and 15. **Only 9 and 11 are untested** — 8, 10 and 12 were answered by the `RSCTRL` control file, which carried our `_PROGRAM_HW_DEFAULTS` through the same validating load and came back unchanged. (That capture existed an hour before a probe was proposed for it: a diff finding nothing is a positive result, and we nearly paid a card write to re-measure it.) A contiguous 0–14 remains a *prediction*. **Also: the load path is the only authority** — s3ked's `PHEADER` write path accepts 15, 23 and 40 unvalidated, so a value poked into a live machine sticks while the same value in a volume is zeroed on load. Related: s3ked notes their `params.py` declares these fields 0..255 from the documentation, which the machine plainly does not accept — a UI offering 0..255 offers values that are silently discarded. We do not expose them, so no change here. §AKAIRESAVE |
| **AKAI: `K_FREQ` accepts 22 where the spec says 0..12** |
| **hw_measure: the analysis window can miss the attack** | A velocity sweep measured `v120` 22 dB below the same configuration measured in isolation, with both captures clean at the noise floor before onset. An interleaved A/B repeat showed the configuration is stable to **0.1 dB**, so the sweep's `anchor_offset` landed differently and the 1.0 s window missed the peak. Three explanations were proposed and wrong before the repeat was run. Also: the note gap (0.6 s) is shorter than a cymbal's decay (6–13 s), so low-velocity takes can be quieter than the residue of the previous one. **Fix:** diagnose from the saved WAVs in `~/temp/krzfix2/`, and match the gap to the material. Noted 2026-08-16 |
| ~~KRZ velocity→filter modulation dropped~~ | **fixed 2026-08-16, hardware-verified.** The reader took only the FIRST modulation source slot (`seg[5]`/`seg[6]`); a bank routing everything through Src2 (`seg[10]`/`seg[9]`) converted as a static corner, and its cymbals were inaudible on an S3000XL. Reader now takes both; the AKAI writer emits keygroup byte 151. Measured +25.3 dB at v120, reproducible to 0.1 dB over four interleaved A/B pairs. §K2DSP, §AKAIVFR |
| ~~KRZ LFO→filter modulation dropped~~ | **fixed 2026-08-17.** Same defect as the velocity row above, one source along: `LFO1` (114) and `LFO2` (116) on either filter source slot were decoded and thrown away. `models/common.py` has carried `lfo1_to_filter`/`lfo2_to_filter` all along. **Two separate bugs, and the second is the interesting one:** the parser decodes into a private layer object whose fields are copied into `VoiceLayer` one keyword at a time, so the first attempt assigned `cur.lfo1_to_filter`, passed all 412 tests, and delivered **0 of 126** routings to the model — a stray attribute nobody reads, with no error and plausible output. Pinned by `tests/test_krz_layer_fields_reach_the_model.py`, which checks the WIRE (every shared field forwarded, every assignment declared) rather than these two fields. Also unified: slot 1 read `byte/127` while slot 2 read `cents/10800`, two scales for one field; `seg[6]` carries the same taper as `seg[9]`, exact on 4 measured pages (62→3400ct, 46→1800, 42→1400, 17→45). That corrects `filter_env_amount` too, which had been reading a cents-scaled byte as a fraction. §K2DSP |
| **KRZ: 4461 filter routings decoded and discarded — needs a model decision** | Full-corpus census (201 banks, 15451 filter F1 slots, 14134 non-OFF routings): **31.6% go nowhere** because `VoiceLayer` has no field for the source. `ON` 1235, `MWheel` 710, `Breath` 549, `MPress` 546, `Data` 463, `PWheel` 331, `ASR1` 156, `ENV3` 102, then a long tail. **Not a parser bug** — the bytes are read correctly and have nowhere to go, so this is a model question and deliberately not answered by inventing fields at the end of an unattended run. **`ON` is the cheap one and should be split off:** it is a constant-true source, so its depth is a STATIC offset to the corner that folds into `filter_cutoff` with no new field — 1235 routings, 8.7% of all of them, currently converting as an un-shifted filter. The performance controllers (MWheel/Breath/MPress/Data/PWheel, 2599) need a real-time modulation concept the model does not have, and most targets can't express it anyway. Measured 2026-08-17. §K2DSP |
| **KRZ: multi-disk banks are not assembled — the continuation volume reads as an empty bank** | `the continuation volume of a two-disk soundset` is 1.4 MB and decodes to **zero objects** (object table declares 36 bytes). Its sibling `its first disk` (Disk1) reads normally at osize=75484, so this is the **second floppy of one bank**: a K2000 bank too large for one disk continues onto the next, and the continuation volume carries sample DATA whose object headers stayed on disk 1. *Inferred* from same soundset + sequential disk dirs + full-size file with a stub table; not hardware-confirmed. **Reading it needs multi-disk assembly** — concatenating PCM across files against disk 1's headers — which we do not have. Now WARNS instead of reporting success with an empty bank. 1 of 201 corpus banks. Found 2026-08-17 by auditing silent emptiness after eosed flagged the pattern |
| ~~KRZ: programs-only banks reported as bare zeros~~ | **fixed 2026-08-17.** 39 of 201 corpus banks (19.4%) hold programs but no sample or keymap objects — they reference the K2000's ROM soundset, so there is nothing to convert. `one programs-only bank` carries 100 programs and not one sample. Correct behaviour, but it printed `0 preset(s), 0 sample(s)` and exited successfully, identical to a failure. **The diagnostic for exactly this case already existed and could never fire:** its guard was `not samples and (sample_objs or keymap_objs)`, and a programs-only bank is defined by having neither. Guard corrected to `program_objs`; 38 of 39 now explained, 0 false positives across the 162 banks that do carry presets (the 39th is the multi-disk row above) |
| ~~E4B: which field the E4XT pitches from~~ | **ANSWERED 2026-08-17 on hardware, both directions.** `[58-59]` is authoritative for **playback pitch**; `[54-57]` drives the **display**. eosed's mirror pair (identical PCM, sha256-verified on our side) measured PITCH_A at **+817.1 cents** and PITCH_B at **−0.0**, both following the offset and ignoring the stored rate, within half a cent, after calibrating the chain on pure sines (−0.8/−0.8/−0.6 ct). But the rate field is **not inert** — same PCM shows `2.00secs 27500Hz` vs `1.24secs 44100Hz` on Sample Manage, so 'informational' was right about pitch and wrong about the field. **No writer change needed:** `_sample_header` derives both from one `sample.sample_rate`, verified across 22077 headers in 681 files. Also validates `--single-cycle` (baked-rate tuning works precisely because `f58` is emitted for every rate ≠ 44100). **Note:** PITCHCHK.E4B could never have answered this — its two fields agree by construction. §E4BRATE2 |
| **~1265 stale E4B headers in `/home/lentferj/temp` play 817 cents sharp** | rate=27500 with offset=0 — the configuration now measured as +817.1 cents. All date **2026-06-07 to 2026-07-24**, at or before the rate-offset fix; current output is clean. Includes `B010_hw.E4B` / `B011_hw.E4B`, which are cited as reference banks in older notes. Not a code bug — a caution: **do not use a pre-2026-07-24 E4B from temp as reference material.** Measured 2026-08-17 |
| **E4B `E3S1[18-21]`: we write 0, the machine never does** | Every sample on a machine-authored HD0 backup carries a NON-ZERO value there, and mpc2emu always writes 0. The field is confirmed non-deterministic — samples at the same rate hold different values (3 of 4 multi-sample rates disagree internally), so it is not pitch and not a function of rate; likely a checksum or edit-id. **Our files therefore differ from machine-authored ones in a field whose consequences have never been tested.** Probably benign. Recorded so "benign" is known to be an assumption rather than a finding. Confirmed by eosed 2026-08-16 from outside our corpus. §E4BRATE2 |
| **The EOS native disk layout is not parsed** | An E4XT HD0 image holds ~5300 samples in the disk's native EOS structure (32-byte directory records: tag/size/offset/index/name/flags) plus a handful of E4B files. We read only the E4B files — 16 of 5311 tag sites on the test image. Parsing the native layout would open a corpus ~330x larger for every E4B claim we make. Real work, and only worth it if a claim needs the population. Noted 2026-08-16 |
| ~~CD1 option-volume recipes unknown~~ | **derived and verified 2026-08-16.** All 8 deduced from the discs' own fingerprints (frame counts, loop counts, sample rates) and confirmed by rebuild: 7 matched first time; `OPTTRIMT` needed `--trim-tail`, not `--trim` — the volume names are literal, S=start T=tail. Rebuilt disc reproduces **8/8 volumes exactly** with 0 name mismatches |
| **CD2 (limits disc) still carries the two-name defect** | 14 of 115 samples have a directory entry that differs from their header — the leading-space form of the `#` bug fixed 2026-08-16 (`PAD-056 AB2` on disc, `' PAD-056 AB2'` in the header). Under an **ALL PROGS+SAMPLES** load those references do not resolve and the samples never become resident, so `ALPHA PAD`'s black keys are silent. Under **ENTIRE VOLUME** they load fine, which is why an earlier audit cleared it. **Not rebuilt** because its sources need reconstructing: `LIM MB 01` and `LIM MB 02` came from material not present under `~/Samples` or `~/Dokumente` (only the June-era `ALPHAPADTEST.iso` / `ANNENBERG.iso` remain, which would need extracting first), and **`LIM PKS` is synthetic** — 23 programs / 920 keygroups / 24 samples generated to approach the 1006 object pool, with no generator script in `tests/re_banks/`. Deprioritised deliberately: the disc's purpose is the MB and P/K/S limits, both already hardware-confirmed (967+39=1006, overflow half-loads), and CD0/CD1 carry the material that is actually being auditioned. Noted 2026-08-16 |
| ~~AKAI writer gives one sample two names~~ | **fixed and hardware-confirmed 2026-08-16.** The directory entry was sanitised with `safe_filename` (a HOST filesystem helper): `#` became `_`, and `_` is absent from the AKAI charset so it encoded as a space. Under `ALL PROGS+SAMPLES` — which resolves by the directory name — 15 samples never loaded and every sharp was silent, while every disc-side check passed because zones reference the header form. Now writes the AKAI name into both fields; `.` needs no escaping either. Confirmed on an S3000XL: 30 samples resident, 15 containing `#`. A boundary warning is on `safe_filename` itself, since that is where the next person reaches. §AKAINAME |
| **`ram_vs_disc.py` joins RAM to disc by program INDEX** | The machine's program list can carry leftovers (`clear_memory` leaves a `TEST PROGRAM` at index 0), which shifts every subsequent index and makes the whole comparison report spurious `ONLY IN RAM` rows. It should join by NAME and carry the machine's index alongside — the same distinction already applied to the K2000 diff, and the same programs-versus-position ambiguity that produced an off-by-13 earlier the same day. Noted 2026-08-16 |
| **A card swapped while the sampler is powered leaves a STALE directory** | `select_volume` does not force a directory re-read; `select_drive` does. Reading the machine after a swap without forcing the re-read describes the PREVIOUS card — 30 samples with 0 `#` from the old disc versus 30 with 15 from the one actually in the drive. Any tool of ours that reads AKAI media state must force the re-read or it will confidently describe a disc that is not there. Found jointly with s3ked 2026-08-16. §AKAINAME |
