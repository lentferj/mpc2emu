...
| **AKAI programs are written with a fixed filter and envelope** | `akai_s3000_writer._keygroup` hardcodes `FILFRQ 99` (fully open), amp attack 0 / decay 50 / sustain 99 / release 45 and the same again for the filter envelope, so **every converted AKAI program discards the source preset's cutoff and envelope entirely**. Not a rounding loss -- the values never reach the file. Fixing it needs the map from hertz and seconds onto the machine's 0-99 scales, which the specification does not give: it states each range and no unit. The measurement tooling for it is built in the sibling **s3ked** (`~/git-repos/s3ked`, `probes/calibrate.py` + `s3k/measure.py`, procedure in its `docs/re_procedures/calibration.md`), and the source disc it needs is `tests/re_banks/gen_akai_cal_disc.py` here. **Blocked on:** the S3000XL, and on s3ked's own offset confirmation before any sweep can be believed. Found 2026-08-08 |
...
| **AKAI auxiliary file types are carried but not read** | Every real library volume also holds `.T` (take list), `.X` (effects), `.D` (drum inputs) and `.M3` (multi) files — 825 of them across the 20-disc corpus. They are named correctly, listed, and survive a disk-image round trip byte-identical, but nothing reads their contents, so a multi's program assignments and an effects file's settings are lost in conversion. `.M3` is the closest to worth doing (it is the AKAI equivalent of a multi/performance) and no reference documents any of them. Low priority: E4B multisetups are not written either, so this is not a parity gap with the other formats. Noted 2026-08-06 |
...
| **AKAI stereo samples are mixed down to mono** | E4B and KRZ write stereo; the AKAI writer averages both sides. There is no stereo flag on disk — the sample header's `0x88` "stereo partner" is annotated *internal* (a RAM pointer) — and eight real library discs pair stereo halves by a **`-L` / `-R` name suffix** (`PF BDF C 0-L`). So writing stereo means emitting name-suffixed pairs and trusting the sampler to pair them. **Blocked on:** hardware — whether it pairs automatically, and what happens to the 12-character name budget once two characters go to the suffix. Noted 2026-08-05. §AKAI_S3000_FORMAT |
| **No per-note polyphony warning for AKAI** | `polyphony_warnings` has measured per-note voice limits for the E4XT (32) and K2000 (24) and none for the S3000XL, so an AKAI conversion gets no warning when a preset stacks more layers on one note than the hardware will sound. The program common's polyphony field is 1–32, but that is a *program* setting, not a measured per-note figure, and guessing it would put an invented number in a user-facing warning. **Blocked on:** the same bench measurement the other two got. Noted 2026-08-05 |
...
| **AKAI S3000 writer: the `??` regions are still guesses** | Cross-verified against akaiutil and against 33 real library discs. A byte-by-byte audit against 11 238 real samples and 4 433 real programs (2026-08-07) found **26 offsets holding a value no real disc writes; 25 are now fixed** — the 0xFFFF null pointer, block RAM addresses, play range, space-filled unused zone names. The one left is sample `0x18` (part of `locat`, an absolute RAM address with no 'none' sentinel). Zero tags and OS version 0x1100 differ from real discs **deliberately**; see §AKAI_S3000_FORMAT. What remains unverified is the documented `??` regions, zero-filled. **First hardware check:** compare against a program the S3000XL saved itself. §AKAI_S3000_FORMAT |
...
| **AKAI: `K_FREQ` accepts 22 where the spec says 0..12** | The keygroup key-follow field took a write of 22 and read it back. Accepting a write is NOT the same as the value being effective — this machine does not bounds-check everything (byte 151 clamps at ±50, this apparently does not). s3ked's §43 fitted the law at r² 0.99890 with 12 giving one octave per octave. **The strong test** is whether the corner shift differs between 12 and 22, i.e. whether the law extrapolates or flattens. Raised by s3ked 2026-08-16 |
