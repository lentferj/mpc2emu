...
| **`sampledir_parser` names samples by its own rules, and the head-vs-tail fix never reached it** | It uses none of the three helpers the XPM path now shares: `name = w.stem[:16]` plus a private counter (`parsers/sampledir_parser.py:128`), so no `_safe_name`, no `_prefers_tail` (`12f74ee`), no `_unique_sample_name` (`cbe6f10`). **(1) Head-only is backwards for the usual shape of a sample folder.** A per-note export shares the instrument prefix and differs at the END — `Minibrute 12-000-036-c1`, `…-040-e1`, `…-084-c6` — so the head keeps the part that identifies nothing, exactly the mirror of the drum-kit case `12f74ee` fixed. Measured over a real 5256-folder sample library, counting the 1942 folders with 4+ WAVs: the **tail** keeps more distinct names in **1064** of them, the head in **37**, no difference in 841. Forced renames: **54 031** under today's head-only rule, 5 254 under tail-only, **4 710** under a per-folder choice. One 156-file folder collapses **144** names today and **0** under the tail; a 455-file one, 428 against 25. `_prefers_tail` already exists and decides exactly this — it is simply never called here. **(2) Names are not sanitised, and the cut is by BYTES.** `Bäss Ünïcode C3.wav` reaches a written E4B as `B?ss ?n?code ` — the trailing `C3`, the one part naming the note, is gone, because two-byte characters fill the 16-byte field early; a literal TAB in a filename passes straight into the name field. `_safe_name` replaces both and counts characters. Rare in this library (6 of 204 935 files, 3 folders) but silent when it happens. **(3) Its counter can outgrow the field:** `f"{name[:14]}{i}"` is 17 characters once `i` reaches 100. The writer truncates and the names stay distinct, so nothing is lost — but `_unique_sample_name` already handles that correctly. No audio is at risk today: unlike the pre-`cbe6f10` XPM path, this counter does check `used_names`. The fix is to call the three existing helpers instead of the private rules. Found via VinSamLib 2026-08-04 |
