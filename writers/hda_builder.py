...
def _print_zuluscs_instructions(out: Path, entries: List[dict]) -> None:
    ...
    print()
    print("     A SCSI ID is bound when the bus is scanned AT BOOT. Renaming")
    print("     the file while the sampler is powered on changes nothing until")
    print("     it is power-cycled -- and the machine will go on showing the")
    print("     old ID, so it looks like the rename failed. This is ordinary")
    print("     SCSI: the initiator enumerates the bus once, at boot.")
    print()
